...
import com.syncleus.aethermud.game.core.intermud.IMudClient;
import com.syncleus.aethermud.game.core.intermud.cm1.CM1Server;
import com.syncleus.aethermud.game.core.intermud.i3.IMudInterface;
...
import com.syncleus.aethermud.game.core.smtp.SMTPserver;
import com.syncleus.aethermud.game.core.threads.CMRunnable;
import com.syncleus.aethermud.game.core.threads.ServiceEngine;
...
public class MUD extends Thread implements MudHost {
    ...
    public static void execExternalRestart() {
        ...
        try {
            ...
            } else if (new File(".\\restart.bat").exists()) {
    ...
    public String executeCommand(String cmd) throws Exception {
        ...
        if (word.equalsIgnoreCase("START") && (V.size() > 1)) {
            ...
            if (what.equalsIgnoreCase("I3")) {
                startIntermud3();
                return "Done";
            } else if (what.equalsIgnoreCase("IMC2")) {
                startIntermud2();
                return "Done";
            } else if (what.equalsIgnoreCase("WEB")) {
                if (V.size() < 3)
        ...
        } else if (word.equalsIgnoreCase("STOP") && (V.size() > 1)) {
            final String what = V.elementAt(1);
        ...
        } else if (word.equalsIgnoreCase("WEBSERVER") && (V.size() > 2)) {
                ...
                while ((!oneStarted) && (System.currentTimeMillis() < timeout)) {
                    ...
                    for (final MUD m : hostMuds) {
                        if (m.state == MudState.STOPPED)
                            numStopped++;
                        else if (m.state != MudState.STARTING)
                            oneStarted = true;
                    }
                                ...
                                synchronized (accessed) {
                                    for (final Iterator<Triad<String, Long, Integer>> i = accessed.iterator(); i.hasNext(); ) {
                                        ...
                                        if ((triad.second.longValue() + LastConnectionDelay) < System.currentTimeMillis())
                                            i.remove();
                                        else if (triad.first.trim().equalsIgnoreCase(address)) {

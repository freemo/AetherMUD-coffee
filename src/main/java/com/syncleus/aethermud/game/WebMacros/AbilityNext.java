...
public class AbilityNext extends StdWebMacro {
    ...
    public String runMacro(HTTPRequest httpReq, String parm, HTTPResponse httpResp) {
        ...
        if (!parms.containsKey("SORTEDBYNAME"))
            a = CMClass.abilities();
        else if (httpReq.getRequestObjects().containsKey("ABILITIESSORTEDBYNAME"))
        ...
        for (; a.hasMoreElements(); ) {
            ...
            if ((className != null) && (className.length() > 0)) {
                ...
                if ((level < 0) && (!unqualifiedOK))
                    okToShow = false;
                else if (CMLib.ableMapper().getSecretSkill(className, false, A.ID()))
                    okToShow = false;
                else if ((flags > 0) && ((A.flags() & flags) != flags))
                    okToShow = false;
                else {
                ...
                if ((level < 0) && (!unqualifiedOK))
                    okToShow = false;
                else if (CMLib.ableMapper().getAllSecretSkill(A.ID()))
                    okToShow = false;
                else if ((flags > 0) && ((A.flags() & flags) != flags))
                    okToShow = false;
                else {

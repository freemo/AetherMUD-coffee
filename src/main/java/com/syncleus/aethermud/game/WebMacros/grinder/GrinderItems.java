...
public class GrinderItems {
    public static String editItem(HTTPRequest httpReq,
        ...
        synchronized (sync.intern()) {
            ...
            else if (itemCode.startsWith("CATALOG-") || itemCode.startsWith("NEWCATA-"))
                I = CMClass.getItem(newClassID);
            else if (M != null)
                ...
                if ((I.isGeneric() || (!generic)))
                    switch (o) {
                        ...
                        case AMMOTYPE: // ammo types
                            ...
                            else if ((I instanceof AmmunitionWeapon) && (!(I instanceof Wand)))
					...
					*/
                        ...
                        case CONTENTSACCESS:
                            ...
                            else if ((I.basePhyStats().sensesMask() & PhyStats.SENSE_INSIDEACCESSIBLE) > 0)
                                I.basePhyStats().setSensesMask(I.basePhyStats().sensesMask() - PhyStats.SENSE_INSIDEACCESSIBLE);
                            break;
                        case BLENDEDVIEW:
                            if (old.equals("on"))
                                I.basePhyStats().setSensesMask(I.basePhyStats().sensesMask() | PhyStats.SENSE_ALWAYSCOMPRESSED);
                            else if ((I.basePhyStats().sensesMask() & PhyStats.SENSE_ALWAYSCOMPRESSED) > 0)
                                I.basePhyStats().setSensesMask(I.basePhyStats().sensesMask() - PhyStats.SENSE_ALWAYSCOMPRESSED);
                            break;
                        case ISKEY: // is key
                            break;

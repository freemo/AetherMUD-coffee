...
public class StdAbility implements Ability {
    ...
    protected int experienceLevels(MOB caster, int asLevel) {
        ...
        if (qualifyingLevel >= 0) {
            ...
            else if (caster.phyStats().level() >= qualifyingLevel)
                adjLevel = (caster.phyStats().level() - qualifyingLevel) + 1;
            else if (caster.phyStats().level() >= lowestQualifyingLevel)
    ...
    protected MOB getTarget(MOB mob, List commands, Environmental givenTarget, boolean quiet, boolean alreadyAffOk) {
        ...
        if ((givenTarget != null) && (givenTarget instanceof MOB))
            ...
            else if (castingQuality(mob, mob) == Ability.QUALITY_BENEFICIAL_SELF)
                target = mob;
            else if (inCombat
            ...
            target = mob;
        else if ((targetName.length() > 0) && (mob.location() != null)) {
    ...
    protected Physical getAnyTarget(MOB mob, List<String> commands, Physical givenTarget, Filterer<Environmental> filter, boolean checkOthersInventory, boolean alreadyAffOk) {
        ...
        if (givenTarget != null)
            ...
            target = mob;
        else if ((targetName.length() == 0) && (mob.isInCombat()) && (abstractQuality() == Ability.QUALITY_MALICIOUS))
            target = mob.getVictim();
        else if (R != null) {
    ...
    protected Item getTarget(MOB mob, Room location, Environmental givenTarget, Item container, List<String> commands, Filterer<Environmental> filter) {
        ...
        if ((target == null)
            ...
            else if ((target == null) || (target instanceof Item)) {
                if (targetName.trim().length() == 0)
                    mob.tell(L("You don't see that here."));
                else if (!CMLib.flags().isSleeping(mob)) // no idea why this is here :(
    ...
    public boolean proficiencyCheck(MOB mob, int adjustment, boolean auto) {
        ...
        else if (CMLib.dice().rollPercentage() > (100 + adjustment))
            return false;
    ...
    protected int[] buildCostArray(MOB mob, int consumed, int minimum) {
        ...
        if ((useMana) && (useMoves) && (useHits))
            divider = 3;
        else if ((useMana) && (useMoves) && (!useHits))
            divider = 2;
        else if ((useMana) && (!useMoves) && (useHits))
            divider = 2;
        else if ((!useMana) && (useMoves) && (useHits))
    ...
    public boolean canBePracticedBy(MOB teacher, MOB student) {
        ...
        if (yourAbility.proficiency() > teacherAbility.proficiency()) {
            ...
            return false;
        } else if (yourAbility.proficiency() > prof75 - 1) {

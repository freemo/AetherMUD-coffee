...
public class CraftingSkill extends GatheringSkill {
    ...
    protected int getBuildingMaterial(int woodRequired, int[][] foundData, int[] compData) {
        ...
        else if ((woodRequired == 0) && (foundData[1][FOUND_CODE] > 0))
            return foundData[1][FOUND_CODE];
        else if ((foundData[0][FOUND_CODE] == 0) && (foundData[1][FOUND_CODE] != 0))
            return foundData[1][FOUND_CODE];
        else
        ...
        if (req2Required > 0) {
            if (req2Desc != null) {
                if (((req2 != null) && (data[1][FOUND_AMT] == 0))
                    ...
                    else if (req2Desc.equalsIgnoreCase("WOODEN"))
    ...
    public boolean isANativeItem(String name) {
        ...
        for (final List<String> recipe : recipes) {
            ...
            for (int n = 0, o = 0; ; n++) {
                ...
                } else if (allExpertiseWords.contains(nw)) {
                    match = true;
                    // wasted real word, wait to match recipe word
                } else if (o == thisOneNameV.size()) {
                    match = false;
                    break;
                } else if (thisOneNameV.get(o).equals("%")) {

...
public class Trap_Trap extends StdAbility implements Trap {
    ...
    public void gas(MOB mob) {
        ...
        else if (mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION, L("<S-NAME> trigger(s) a trap set in <T-NAME>!"))) {
    ...
    public void needle(MOB mob) {
        ...
        else if (mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION, L("<S-NAME> trigger(s) a needle trap set in <T-NAME>!"))) {
            final MOB target = mob;
    ...
    public void blade(MOB mob) {
        ...
        else if (mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION, L("<S-NAME> trigger(s) a blade trap set in <T-NAME>!"))) {
            final MOB target = mob;
    ...
    public void fallInPit(MOB mob) {
        if (CMLib.flags().isInFlight(mob)) {
            ...
            return;
        } else if (CMLib.dice().rollPercentage() < mob.charStats().getSave(CharStats.STAT_SAVE_TRAPS))
            mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION, L("<S-NAME> avoid(s) a trap door beneath <S-HIS-HER> feet."));
        else if (mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION, L("<S-NAME> trigger(s) a trap door beneath <S-HIS-HER> feet! <S-NAME> fall(s) in!"))) {
    ...
    public boolean tick(Tickable ticking, int tickID) {
        ...
        if (tickID == Tickable.TICKID_TRAP_RESET) {
            sprung = false;
            return false;
        } else if (tickID == Tickable.TICKID_TRAP_DESTRUCTION) {
            unInvoke();
            return false;
        }
        return true;

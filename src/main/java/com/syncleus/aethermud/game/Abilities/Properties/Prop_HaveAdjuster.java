...
public class Prop_HaveAdjuster extends Property implements TriggeredAffect {
    ...
    public static final String fixAccoutingsWithMask(String parameters, final String mask) {
        ...
        for (final StringBuffer ID = new StringBuffer(parameters); ((x > 0) && (x < parameters.length())); x++) {
            if (parameters.charAt(x) == '-') {
                ...
                break;
            } else if (parameters.charAt(x) == '+') {
                ID.setCharAt(x, '-');
                parameters = ID.toString();
                break;
            } else if (Character.isDigit(parameters.charAt(x)))
                break;
        }
    ...
    public void adjCharStats(Object[] changes, CharStats charStats) {
        ...
        for (int i = 0; i < changes.length; i += 2) {
            ...
            else if (changes[i] instanceof Character) {
    ...
    public String getStat(String code) {
        ...
        if (code.toUpperCase().startsWith("STAT-")) {
            ...
            return "0";
        } else if (code.equalsIgnoreCase("LEVEL")) {
            int level = 0;
            ...
            if (changes != null) {
                for (int i = 0; i < changes.length; i += 2) {
                    if (changes[i] instanceof Integer) {
                        ...
                        else if (CMParms.indexOf(CharStats.CODES.MAXCODES(), ((Integer) changes[i]).intValue()) >= 0)
                            level += (((Integer) changes[i + 1]).intValue() * 15);
                        else if (CMParms.indexOf(CharStats.CODES.SAVING_THROWS(), ((Integer) changes[i]).intValue()) >= 0)
            ...
            if (changes != null) {
                if (multiplyPhyStats) {
                    for (int c = 0; c < changes.length; c += 2) {
                        ...
                        else if (changes[c + 1] instanceof Double)
    ...
    public void setStat(String code, String val) {
        if (code != null) {
            ...
            } else if (code.equalsIgnoreCase("TONEDOWN")) {
                setStat("TONEDOWN-ARMOR", val);
                setStat("TONEDOWN-WEAPON", val);
                setStat("TONEDOWN-MISC", val);
            } else if (code.equalsIgnoreCase("TONEDOWN-ARMOR")) {
            ...
            } else if (code.equalsIgnoreCase("TONEDOWN-WEAPON")) {
            ...
            } else if (code.equalsIgnoreCase("TONEDOWN-MISC")) {
                final double pct = CMath.s_pct(val);

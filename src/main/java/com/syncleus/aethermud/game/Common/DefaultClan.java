...
import com.syncleus.aethermud.game.Common.interfaces.ClanPosition;
...
public class DefaultClan implements Clan {
    ...
    public void setExp(long newexp) {
        synchronized (expSync) {
            ...
            } else if ((oldxp > exp) && (getClanLevel() > 1)) {
    ...
    public boolean updateClanPrivileges(MOB M) {
        ...
        if (M.playerStats() != null)
            for (final ClanPosition pos : govt().getPositions()) {
                ...
                } else if (M.playerStats().getTitles().contains(title))
    ...
    public String getDetail(MOB mob) {
        ...
        } else if (getBasicRequirementMask().length() > 0)
        ...
        try {
            ...
            if (activeMembers < minimumMembers) {
                if (!isSafeFromPurge()) {
                    ...
                    update();
                } else if (getStatus() != CLANSTATUS_ACTIVE)
            ...
            if (govt().getAutoPromoteBy() != AutoPromoteFlag.NONE) {
                ...
                } else if (basePromoteBy == AutoPromoteFlag.RANK_OVERWRITE) {
                    overWrite = true;
                ...
                for (final Enumeration<ClanVote> e = votes(); e.hasMoreElements(); ) {
                    ...
                    if (CV.voteStatus == VSTAT_STARTED) {
                        ...
                        if (System.currentTimeMillis() > endsOn)
                            voteIsOver = true;
                        else if (CV.votes.size() == numVotes)
                            voteIsOver = true;
                        if (voteIsOver) {
                    ...
                    } else if (System.currentTimeMillis() > endsOn) {
                        updateVotes = true;

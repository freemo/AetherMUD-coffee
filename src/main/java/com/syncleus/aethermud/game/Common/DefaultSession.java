...
public class DefaultSession implements Session {
            ...
            case TELNET_TERMTYPE:
                if (dataSize >= 1) {
                    ...
                    if (suboptionData[0] == 0) {
                        ...
                        } else if (terminalType.equalsIgnoreCase("ANSI"))
                            changeTelnetMode(rawout, TELNET_ECHO, true);
                        else if (terminalType.startsWith("GIVE-WINTIN.NET-A-CHANCE")) {
                            rawOut("\n\r\n\r**** Your MUD Client is Broken! Please use another!!****\n\r\n\r");
                            rawout.flush();
                            CMLib.s_sleep(1000);
                            rawout.close();
                        } else if (terminalType.toLowerCase().startsWith("mushclient") && (!CMSecurity.isDisabled(CMSecurity.DisFlag.MXP))) {
                            negotiateTelnetMode(rawout, TELNET_MXP);
                            mxpSupportSet.remove("+IMAGE");
                            mxpSupportSet.remove("+IMAGE.URL");
                            mxpSupportSet.add("-IMAGE.URL");
                        } else if (terminalType.toLowerCase().equals("simplemu")) {
                    ...
                    } else if (suboptionData[0] == 1) // Request for data.
    ...
    public void handleIAC()
        ...
        switch (c) {
            ...
            case TELNET_SB: {
                ...
                while (((last = readByte()) != -1)
                    ...
                    if (subOptionStream.size() > 1024 * 1024 * 5) {
                        killFlag = true;
                        return;
                    } else if (last == TELNET_IAC) {
                        last = readByte();
                        if (last == TELNET_IAC)
                            subOptionStream.write(TELNET_IAC);
                        else if (last == TELNET_SE)
                            break;
                    } else
            ...
            case TELNET_DO: {
                ...
                } else if (!mightSupportTelnetMode(last))
                    changeTelnetMode(last, false);
                else if (!getServerTelnetMode(last))
    ...
    protected int nonBlockingIn(boolean appendInputFlag)
        ...
        try {
            ...
            else if ((c == TELNET_IAC) || ((c & 0xff) == TELNET_IAC))
                handleIAC();
            else if (c == '\033')
                handleEscape();
            else {
        ...
        if ((inStr.length() > 3) && (inStr.substring(0, 2).equals("#$"))) {
            if (inStr.substring(0, 3).equals("#$#")) {
                ...
                if (CMLib.protocol().mcp(this, inStr, mcpKey, mcpSupported, mcpKeyPairs))
                    return false;
            } else if (inStr.substring(0, 3).equals("#$\""))
                inStr.delete(0, 3);
        }
        return true;
    }
    ...
    public void loginSystem() {
        try {
            if ((loginSession == null) || (loginSession.reset())) {
                ...
                setStatus(SessionStatus.LOGIN);
            } else if (!loginSession.skipInputThisTime()) {
    ...
    private static enum SESS_STAT_CODES {

...
public class CMSecurity {
    ...
    private static final Map<String, String[]> getSpecialEnableMap(final String anyFlag) {
        ...
        } else if (flag.startsWith(XABLE_PREFIX_EXPERTISE)) {
        } else if (flag.startsWith(XABLE_PREFIX_COMMAND)) {
        } else if (flag.startsWith(XABLE_PREFIX_RACE)) {
            return instance().racEnaVars;
        } else if (flag.startsWith(XABLE_PREFIX_CHARCLASS)) {
            return instance().clsEnaVars;
        } else if (flag.startsWith(XABLE_PREFIX_FACTION)) {
        }
        return null;
    }
            ...
            for (int b = 0; b < banned.size(); b++) {
                ...
                if (str.length() > 0) {
                    if (str.equals("*") || ((str.indexOf('*') < 0)) && (str.equals(uplogin)))
                        return true;
                    else if (str.startsWith("*") && str.endsWith("*") && (uplogin.indexOf(str.substring(1, str.length() - 1)) >= 0))
                        return true;
                    else if (str.startsWith("*") && (uplogin.endsWith(str.substring(1))))
                        return true;
                    else if (str.endsWith("*") && (uplogin.startsWith(str.substring(0, str.length() - 1))))
                        return true;
                }
            }
        }
    ...
    public Object parseSecurityFlag(String s) {
        ...
        } else if (journalFlags.contains(s))
            return s;
        else if (s.startsWith("KILL") && journalFlags.contains(s.substring(4)))
            return s.substring(4);
        else if (s.startsWith("KILL") && s.endsWith("S") && journalFlags.contains(s.substring(4, s.length() - 1)))
            return s.substring(4, s.length() - 1);
        else if (s.endsWith("S") && journalFlags.contains(s.substring(0, s.length() - 1)))
    ...
    public final SecGroup createGroup(String name, final List<String> set) {
        ...
        for (int v = 0; v < set.size(); v++) {
            ...
            else if (o instanceof SecGroup)
                newGroups.add((SecGroup) o);
            else if (o instanceof SecPath)
                newPaths.add((SecPath) o);
            else if (o instanceof SecFlag)

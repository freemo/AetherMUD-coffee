...
public class LongSet implements Set<Long> {
    ...
    public int getIntIndex(int x) {
        ...
        while (start <= end) {
            ...
            else if ((intArray[mid] & NEXT_FLAG) > 0) {
                if (x > intArray[mid + 1])
                    start = mid + 2;
                else if (x <= intArray[mid + 1])
                    return mid;
    ...
    public int getLongIndex(long y) {
        ...
        while (start <= end) {
            ...
            else if ((longArray[mid] & NEXT_FLAGL) > 0) {
                if (y > longArray[mid + 1])
                    start = mid + 2;
                else if (y <= longArray[mid + 1])
                    return mid;
            } else if (y > longArray[mid])
                start = mid + 1;
            else
                return mid;
        }
            ...
            if (index1 < longArray.length) {
                if (index1 >= 0) {
                    if (x2 < (longArray[index1] & LONG_BITS)) {
                        ...
                        return;
                    } else if (((longArray[index1] & NEXT_FLAGL) > 0) && ((longArray[index1] & LONG_BITS) >= x1)) {
                        need = 0;
                        longArray[index1] = x1 | NEXT_FLAGL;
                    } else if ((x2 >= (longArray[index1] & LONG_BITS)) && ((longArray[index1] & NEXT_FLAGL) == 0)) {
                        need = 1;
                    }
                }
            }
    ...
    private void addLong(long x) {
        ...
        for (int i = index; i < end; i++) {
            if ((longArray[i] & NEXT_FLAGL) > 0) {
                ...
                i++;
            } else if (x == longArray[i])
                return;
            else if (x == longArray[i] - 1) {
                growLongArray(i, 1);
                longArray[i] = x | NEXT_FLAGL;
                consolodateLongs();
                return;
    ...
    private void addInt(int x) {
        ...
        for (int i = index; i < end; i++) {
            if ((intArray[i] & NEXT_FLAG) > 0) {
                ...
                i++;
            } else if (x == intArray[i])
                return;
            else if (x == intArray[i] - 1) {
                growIntArray(i, 1);
                intArray[i] = x | NEXT_FLAG;
                consolodateInts();
                return;
            ...
            if (index1 < intArray.length) {
                if (index1 >= 0) {
                    if (x2 < (intArray[index1] & INT_BITS)) {
                        ...
                        return;
                    } else if (((intArray[index1] & NEXT_FLAG) > 0) && ((intArray[index1] & INT_BITS) >= x1)) {
                        need = 0;
                        intArray[index1] = x1 | NEXT_FLAG;
                    } else if ((x2 >= (intArray[index1] & INT_BITS)) && ((intArray[index1] & NEXT_FLAG) == 0)) {
                        need = 1;
                    }
                }
            }

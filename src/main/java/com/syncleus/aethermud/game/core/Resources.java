...
public class Resources {
    ...
    public static final List<String> getFileLineVector(final StringBuffer buf) {
        ...
        for (int i = 0; i < buf.length(); i++) {
            if (((buf.charAt(i) == '\n') && (i < buf.length() - 1) && (buf.charAt(i + 1) == '\r'))
                ...
                str.setLength(0);
            } else if ((buf.charAt(i) == '\r') || (buf.charAt(i) == '\n')) {
                V.addElement(str.toString());
                str.setLength(0);
            } else
    ...
    public static final Map<String, Object> getPersonalMap(final Object key, final boolean createIfNecc) {
        ...
        @SuppressWarnings("unchecked")        final Map<Object, Map<String, Object>> rsc = (Map<Object, Map<String, Object>>) r()._getResource("SYSTEM_PERSONAL_MAPS");
        if (rsc != null) {
            synchronized (rsc) {
    ...
    public static final void removePersonalMap(final Object key) {
        @SuppressWarnings("unchecked")        final Map<Object, Map<String, Object>> rsc = (Map<Object, Map<String, Object>>) getResource("SYSTEM_PERSONAL_MISC");
        if (rsc != null) {
            synchronized (rsc) {
                rsc.remove(key);
            }

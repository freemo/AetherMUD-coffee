...
public class StdRace implements Race {
    ...
    public boolean okMessage(final Environmental myHost, final CMMsg msg) {
        if ((msg.target() == myHost)
            ...
            if (uncharmable()
                ...
                return false;
            } else if (naturalAbilImmunities.contains(msg.tool().ID())) {
                return false;
            }
        }
        return true;
            ...
            for (int i = 1; i < 11; i++) {
                ...
                switch (i) {
                    ...
                    case 9:
                        ...
                        else if (bodyMask()[Race.BODY_GILL] > 0)
    ...
    public DeadBody getCorpseContainer(MOB mob, Room room) {
        ...
        } else if ((room != null) && (room.isContent(bodyI))) {
    ...
    public Race mixRace(final Race race, final String newRaceID, final String newRaceName) {
        ...
        for (int i = 0; i < aging.length; i++) {
            ...
            } else if (((aging[i] == Race.YEARS_AGE_LIVES_FOREVER) && (race2.getAgingChart()[i] == 0))
                || ((aging[i] == 0) && (race2.getAgingChart()[i] == Race.YEARS_AGE_LIVES_FOREVER)))
                aging[i] = 0;
            else if ((aging[i] == Race.YEARS_AGE_LIVES_FOREVER))
                aging[i] = race2.getAgingChart()[i] * 2;
            else if ((race2.getAgingChart()[i] == Race.YEARS_AGE_LIVES_FOREVER))
                aging[i] = aging[i] * 2;
            else {
        ...
        for (final long wornCode : Wearable.CODES.ALL()) {
            if (wornCode != Wearable.IN_INVENTORY) {
                ...
                } else if (CMath.bset(race1worn, wornCode) && CMath.bset(race2worn, wornCode))
                    finalWear = finalWear | wornCode;
                else if (CMath.bset(race1worn, wornCode))
                    finalWear = finalWear | wornCode;
                else if (toggle) {
                    finalWear = finalWear | wornCode;
                    toggle = !toggle;
        ...
        for (int i = 0; i < Race.BODYPARTSTR.length; i++) {
            ...
            } else if ((race1.bodyMask()[i] <= 0) && (race2.bodyMask()[i] >= 0))
        ...
        for (final int i : CharStats.CODES.ALLCODES()) {
            ...
            } else if (CharStats.CODES.isBASE(i)) {

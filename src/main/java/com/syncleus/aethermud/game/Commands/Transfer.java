...
public class Transfer extends At {
    ...
    public boolean execute(MOB mob, List<String> commands, int metaFlags)
        ...
        if (cmd.toString().equalsIgnoreCase("inventory")) {
            room = mob.location();
            inventoryFlag = true;
        } else if (cmd.toString().equalsIgnoreCase("here") || cmd.toString().equalsIgnoreCase("."))
            room = mob.location();
        else if (CMLib.map().getRoom(cmd.toString()) != null)
            room = CMLib.map().getRoom(cmd.toString());
        else if (CMLib.directions().getDirectionCode(cmd.toString()) >= 0)
        ...
        for (int i = 0; i < V.size(); i++) {
            ...
            } else if (V.get(i) instanceof MOB) {

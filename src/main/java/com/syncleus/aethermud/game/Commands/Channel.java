...
import com.syncleus.aethermud.game.Libraries.interfaces.ChannelsLibrary.ChannelMsg;
...
public class Channel extends StdCommand {
    ...
    public boolean channel(MOB mob, List<String> commands, boolean systemMsg) {
        ...
        if ((commands.size() == 2)
            ...
            return false;
        } else if (flags.contains(ChannelsLibrary.ChannelFlag.PLAYERREADONLY) && (!mob.isMonster())) {
            mob.tell(L("This channel is read-only."));
            return false;
        } else if (flags.contains(ChannelsLibrary.ChannelFlag.ARCHONREADONLY) && (!CMSecurity.isASysOp(mob))) {
            mob.tell(L("This channel is read-only."));
            return false;
        } else

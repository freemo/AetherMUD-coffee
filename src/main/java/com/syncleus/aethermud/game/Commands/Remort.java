...
import com.syncleus.aethermud.game.Abilities.interfaces.ExtendableAbility;
        ...
        for (String thing : allRetains) {
            ...
            if (retainer == null) {
                for (RemortRetain r : RemortRetain.values()) {
                    ...
                    } else if (thing.endsWith(r.name() + "S")) {
                        retainer = r;
                        rest = thing.substring(0, thing.length() - r.name().length() - 1).trim();
                    } else if (thing.startsWith(r.name())) {
                        retainer = r;
                        rest = thing.substring(r.name().length()).trim();
                    } else if (thing.startsWith(r.name() + "S")) {
                        retainer = r;
            ...
            if (retainer != null) {
                ...
                } else if (CMath.isInteger(rest)) {
                    flatAmount = CMath.s_int(rest);
                } else if (CMath.isNumber(rest)) {
                        ...
                        public void executeMsg(Environmental myHost, CMMsg msg) {
                            if ((msg.source() == myHost)
                                ...
                                if ((CMath.numberOfSetBits(tryTheme & Area.THEME_ALLTHEMES)) > 1) {
                                    ...
                                    else if ((tryTheme & Area.THEME_TECHNOLOGY) != 0)

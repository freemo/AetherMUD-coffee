...
public class Say extends StdCommand {
    ...
    public boolean execute(MOB mob, List<String> commands, int metaFlags)
        ...
        if (theCommand.equals("ASK"))
            theWord = "Ask";
        else if (theCommand.equals("YELL"))
            theWord = "Yell";
        else if (theCommand.equals("SAYTO")
            ...
            if (whom.equalsIgnoreCase("self")) {
                ...
                if ((!toFlag) && (target != null)) {
                    if (!(target instanceof MOB))
                        target = null;
                    else if (target.name().toUpperCase().indexOf(whom.toUpperCase()) < 0)
                        target = null;
                    else if ((!target.name().equalsIgnoreCase(whom)) && (whom.length() < 4))
                        target = null;
                }
        ...
        if (!CMSecurity.isDisabled(CMSecurity.DisFlag.AUTOLANGUAGE)) {
            ...
            if ((langTarget != null) && (!mob.isMonster()) && (!CMLib.flags().isAnimalIntelligence(mob))) {
                ...
                else if ((hisL != null) && ((myL == null) || (!hisL.ID().equals(myL.ID())))) {

...
public class Create extends StdCommand {
    ...
    public void exits(MOB mob, List<String> commands)
        ...
        } else if ((reverseExit == null) && (opExit == null) && (opRoom != null)) {
            final int revDirCode = Directions.getOpDirectionCode(direction);
    ...
    public void items(MOB mob, List<String> commands) throws IOException {
        ...
        if (x > 0) {
            ...
            } else if ((!rest.equalsIgnoreCase("room"))
                && (rest.length() > 0)) {
        ...
        } else if (newItem instanceof SpaceObject) {
            int i = 21;
    ...
    public boolean execute(MOB mob, List<String> commands, int metaFlags)
        ...
        if (commandType.equals("EXIT")) {
            ...
            mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL, L("^S<S-NAME> wave(s) <S-HIS-HER> arms...^?"));
            races(mob, commands);
        } else if (commandType.equals("MIXEDRACE")) {
            if (!CMSecurity.isAllowed(mob, mob.location(), CMSecurity.SecFlag.CMDRACES))
                return errorOut(mob);
            mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL, L("^S<S-NAME> wave(s) <S-HIS-HER> arms...^?"));
            ...
            if ((commands.size() < 3) || (CMParms.combine(commands, 2).indexOf('.') < 0)) {
                ...
                if (F != null) {
                    ...
                    return false;
                } else if ((!mob.isMonster()) && (mob.session().confirm(L("Create a new faction with ID/filename: 'resources/@x1' (N/y)? ", name), "N"))) {
            ...
            if (commands.size() < 3) {
                ...
                if ((Q.name().trim().length() == 0) || (Q.duration() < 0)) {
                    ...
                    return false;
                } else if ((CMLib.quests().fetchQuest(Q.name()) != null)
                    && ((mob.isMonster())
            ...
            if (commands.size() < 3) {
                ...
                if (C.name().trim().length() == 0) {
                    ...
                    return false;
                } else if (CMLib.clans().getClan(C.clanID()) != null) {
            ...
            if ((E instanceof Item)
                ...
                if ((E instanceof MOB)
                    ...
                    commands.add(1, "MOB");
                    execute(mob, commands, metaFlags);
                } else if ((lastWord != null) && (CMLib.directions().getGoodDirectionCode(lastWord) >= 0)) {
                    ...
                    if ((E != null) && (E instanceof Room)) {
                        ...
                        commands.add(allWord);
                        execute(mob, commands, metaFlags);
                    } else if ((E != null) && (E instanceof Area)) {
                        commands = new Vector<String>();
                        commands.add("CREATE");
                        commands.add("AREA");
                        commands.add(lastWord);
                ...
                } else if (CMath.isInteger(firstWord)
                    && (lastWord != null)) {
                    ...
                    } else if (CMLib.english().numPossibleGold(null, firstWord + " " + theRest) > 0) {
                        commands.add(1, "ITEM");
                ...
                } else if (CMClass.getRace(allWord) != null) {
                    commands.add(1, "MOB");
                    execute(mob, commands, metaFlags);
                } else {

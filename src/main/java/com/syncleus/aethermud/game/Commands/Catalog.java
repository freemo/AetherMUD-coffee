...
import com.syncleus.aethermud.game.Libraries.interfaces.CatalogLibrary.CatalogKind;
import com.syncleus.aethermud.game.Locales.interfaces.Room;
import com.syncleus.aethermud.game.MOBS.interfaces.MOB;
import com.syncleus.aethermud.game.core.*;
import com.syncleus.aethermud.game.core.collections.XVector;
import com.syncleus.aethermud.game.core.interfaces.DBIdentifiable;
...
public class Catalog extends StdCommand {
    ...
    public Enumeration<String> getRoomSet(MOB mob, String which) {
        ...
        else if (which.equalsIgnoreCase("AREA"))
            rooms = mob.location().getArea().getProperRoomnumbers().getRoomIDs();
        else if (which.equalsIgnoreCase("WORLD"))
    ...
    public boolean execute(final MOB mob, List<String> commands, int metaFlags)
        ...
        if ((commands != null) && (commands.size() > 1)) {
            ...
            if (checkUserRoomSetEntry(commands)) {
                ...
                if ((mob.session() != null)
                    ...
                    for (; rooms.hasMoreElements(); ) {
                        ...
                        for (final CatalogLibrary.RoomContent content : contents) {
                            ...
                            } else if ((P instanceof Item) && (whatKind != CatalogKind.MOB)) {
                                if (catalog(R, mob, P)) {
                                    content.flagDirty();
                                    dirty = true;
                                }
            ...
            } else if (commands.get(0).equalsIgnoreCase("CATAGORY") || commands.get(0).equalsIgnoreCase("CATEGORY")) {
                commands.remove(0);
                ...
                if (commands.size() > 0) {
                    ...
                    if (ID.length() > 0) {
                        if (ID.equalsIgnoreCase("LIST")) {
                            ...
                            return false;
                        } else if ((!CMParms.contains(CMLib.catalog().getItemCatalogCatagories(), ID))
                ...
                if ((ID.equalsIgnoreCase("CATEGORIES")
                    ...
                    mob.tell(listCatagories(CatalogKind.OBJECT));
                    return false;
                } else if ((whatKind == CatalogKind.OBJECT) || (whatKind == CatalogKind.MOB)) {
                ...
                else if (ID.equalsIgnoreCase("everydamnitem"))
                    del = CMLib.catalog().getCatalogItems();
                else if (ID.equalsIgnoreCase("everydamnthing")) {
            ...
            } else if (commands.get(0).equalsIgnoreCase("EDIT")) {
                commands.remove(0);
                final CatalogKind whatKind = getObjectType(commands);
                final String ID = CMParms.combine(commands, 0);
                final Physical P = findCatalog(whatKind, ID, false);
            ...
            } else if ((commands.get(0).equalsIgnoreCase("SCAN"))
            ...
            } else if ((commands.get(0).equalsIgnoreCase("OVERLOOK"))
                ...
                if (checkUserRoomSetEntry(commands)) {
                    ...
                    mob.tell(L("Scan what?"));
            } else if (commands.get(0).equalsIgnoreCase("CLEAN")) {
                commands.remove(0);
                if (checkUserRoomSetEntry(commands)) {
                    final String which = commands.get(0).toLowerCase();
                    final Enumeration<String> rooms = getRoomSet(mob, which);

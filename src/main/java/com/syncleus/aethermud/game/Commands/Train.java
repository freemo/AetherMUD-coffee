...
import com.planet_ink.game.core.collections.StringXVector;
...
public class Train extends StdCommand {
    ...
    public boolean execute(MOB mob, List<String> commands, int metaFlags)
        ...
        if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSTRAINING)) && (abilityCode < 0)) {
            for (final Enumeration<CharClass> c = CMClass.charClasses(); c.hasMoreElements(); ) {
                ...
                if ((C.name().toUpperCase().startsWith(abilityName.toUpperCase()))
                    ...
                    break;
                } else if ((C.qualifiesForThisClass(mob, true))
                    && (CMLib.login().isAvailableCharClass(C)))
        ...
        if (abilityCode < 0) {
            ...
            else if ("MANA".startsWith(abilityName.toUpperCase()))
                abilityCode = TRAIN_MANA;
            else if ("MOVE".startsWith(abilityName.toUpperCase()))
                abilityCode = TRAIN_MOVE;
            else if ("GAIN".startsWith(abilityName.toUpperCase()))
                abilityCode = TRAIN_GAIN;
            else if ("PRACTICES".startsWith(abilityName.toUpperCase()))

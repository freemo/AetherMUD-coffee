...
import com.syncleus.aethermud.game.core.CMSecurity.SecFlag;
import com.syncleus.aethermud.game.core.collections.*;
import com.syncleus.aethermud.game.core.interfaces.*;
import com.syncleus.aethermud.game.core.interfaces.ShopKeeper.ShopPrice;
...
public class ListCmd extends StdCommand {
    ...
    public StringBuilder roomPropertyDetails(Session viewerS, Area A, String rest) {
        ...
        else if (rest.trim().equalsIgnoreCase("area"))
            return roomPropertyDetails(viewerS, A.getMetroMap(), null);
        else if (rest.trim().equalsIgnoreCase("world"))
            return roomPropertyDetails(viewerS, CMLib.map().rooms(), null);
        else if (rest.trim().toLowerCase().startsWith("area "))
            return roomPropertyDetails(viewerS, A.getMetroMap(), rest.trim().substring(5).trim());
        else if (rest.trim().toLowerCase().startsWith("world "))
    ...
    private String getShopPrice(Room R, MOB buyer, ShopKeeper SK, Environmental E) {
        ...
        try {
            if (SK instanceof MOB)
                seller = (MOB) SK;
            else if ((SK instanceof Item) && (((Item) SK).owner() instanceof MOB))
                seller = (MOB) ((Item) SK).owner();
            else if ((SK instanceof Ability) && (((Ability) SK).affecting() instanceof MOB))
    ...
    public StringBuffer getStuff(MOB mob, List<String> commands, int start, Enumeration<Room> r) {
        ...
        } else if (who.equalsIgnoreCase("RESOURCE")
        ...
        } else if ((who.equals("EXIT "))
            || (who.equals("EXITS"))) {
            exitOnly = true;
            commands.remove(start);
            rest = CMParms.combine(commands, start);
        } else if ((who.equals("ITEM"))
            || (who.equals("ITEMS"))) {
            itemOnly = true;
            commands.remove(start);
            rest = CMParms.combine(commands, start);
        } else if ((who.equals("MOB"))
            || (who.equals("MOBS"))) {
            mobOnly = true;
            commands.remove(start);
            rest = CMParms.combine(commands, start);
        } else if ((who.equals("SHOP"))
            || (who.equals("SHOPS"))) {
            shopOnly = true;
            commands.remove(start);
            rest = CMParms.combine(commands, start);
        } else if (who.equals("MOBMASK")
            || who.equals("MOBMASK=")) {
            mobOnly = true;
            zapperMask = true;
            commands.remove(start);
                            ...
                            if ((!mobOnly) && (!roomOnly) && (!exitOnly)) {
                                ...
                                if (SK != null) {
                                    for (final Iterator<Environmental> i = SK.getShop().getStoreInventory(); i.hasNext(); ) {
                                        ...
                                        } else if ((zapperMask) && (E instanceof MOB) && (mobOnly)) {
                                            if (CMLib.masking().maskCheck(compiledZapperMask, E, true)) {
                                                lines.append("^!" + CMStrings.padRight(cataMark(E) + E.name(), 17) + "^N| ");
                                                lines.append("SHOP: " + cataMark(M) + M.name(mob) + "^N");
                                                lines.append(" (^<LSTROOMID^>" + CMLib.map().getDescriptiveExtendedRoomID(R) + "^</LSTROOMID^>)");
                                        ...
                                        } else if ((zapperMask2) && (E instanceof Item) && (itemOnly)) {
                                        ...
                                        } else if ((zapperMask2) && (E instanceof MOB) && (mobOnly)) {
                                            if (CMLib.masking().maskCheck(rest, E, true)) {
                                                lines.append("^!" + CMStrings.padRight(cataMark(E) + E.name(), 17) + "^N| ");
                                                lines.append("SHOP: " + cataMark(M) + M.name(mob) + "^N");
                                                lines.append(" (^<LSTROOMID^>" + CMLib.map().getDescriptiveExtendedRoomID(R) + "^</LSTROOMID^>)");
    ...
    public StringBuilder listScripts(Session viewerS, MOB mob, List<String> cmds) {
        ...
        } else if (rest.equalsIgnoreCase("DETAILS")) {
            final int COL_LEN1 = CMLib.lister().fixColWidth(30.0, viewerS);
        ...
        } else if (rest.equalsIgnoreCase("CUSTOM")) {
    ...
    public StringBuilder listRaceCats(Session viewerS, Enumeration<Race> these, List<String> commands) {
        ...
        } else if ((wiki == WikiFlag.WIKILIST) || (wiki == WikiFlag.WIKIHELP)) {
    ...
    public StringBuilder listTicks(Session viewerS, String whichGroupStr) {
        ...
        if ("ACTIVE".startsWith(whichGroupStr.toUpperCase()) && (whichGroupStr.length() > 0))
            activeOnly = true;
        else if ("PROBLEMS".startsWith(whichGroupStr.toUpperCase()) && (whichGroupStr.length() > 0)) {
        ...
        } else if (CMath.isInteger(whichGroupStr) && (whichGroupStr.length() > 0)) {
            whichGroups = new HashSet<Integer>();
            whichGroups.add(Integer.valueOf(CMath.s_int(whichGroupStr)));
        } else if (whichGroupStr.length() > 0) {
    ...
    public String listRecipes(MOB mob, String rest) {
        ...
        if (rest.trim().length() == 0) {
            ...
            else if (!(A instanceof CraftorAbility))
    ...
    public String getSpaceObjectType(final SpaceObject obj) {
        ...
        if ((obj instanceof Physical) && (!(obj instanceof SpaceShip)) && CMLib.flags().isLightSource(((Physical) obj)))
            ...
            type = "Ship";
        else if ((obj instanceof Area) && (!(obj instanceof SpaceShip)) && (obj.radius() > (SpaceObject.Distance.MoonRadius.dm + 10000L)))
            type = "Planet";
        else if ((obj instanceof Area) && (!(obj instanceof SpaceShip)) && (obj.radius() <= (SpaceObject.Distance.MoonRadius.dm + 10000L)))
            type = "Moon";
        else
    ...
    public String listSpace(MOB mob, List<String> commands) {
        ...
        for (int i = 1; i < commands.size(); i++) {
            ...
            } else if (L("ORDERBY").startsWith(s)) {
                if (i < commands.size() - 1) {
                    i++;
                    s = commands.get(i);
                    int end = i;
    ...
    public void listLog(MOB mob, List<String> commands) {
        ...
        for (int i = 1; i < commands.size(); i++) {
            ...
            } else if ((s.equalsIgnoreCase("back") || (s.equalsIgnoreCase("last")) || (s.equalsIgnoreCase("tail")))
                && (i < (commands.size() - 1))) {
                s = commands.get(i + 1);
                if (CMath.isInteger(s)) {
                    i++;
        ...
        if (commands.size() > 0) {
            ...
            while (commands.size() > 0) {
                if (commands.get(0).toString().equalsIgnoreCase("sortby")) {
                    ...
                    addTos = sortBys;
                } else if (commands.get(0).toString().equalsIgnoreCase("cols") || commands.get(0).toString().equalsIgnoreCase("columns")) {
                    commands.remove(0);
    ...
    public void listSessions(MOB mob, List<String> commands) {
        ...
        if (sort.length() > 0) {
            if ("STATUS".startsWith(sort))
                sortNum = 1;
            else if ("VALID".startsWith(sort))
                sortNum = 2;
            else if (("NAME".startsWith(sort)) || ("PLAYER".startsWith(sort)))
                sortNum = 3;
            else if (("IP".startsWith(sort)) || ("ADDRESS".startsWith(sort)))
                sortNum = 4;
            else if (("IDLE".startsWith(sort)) || ("MILLISECONDS".startsWith(sort)))
                sortNum = 5;
        }
        if (sortNum < 0)
            sorted = broken;

...
import com.syncleus.aethermud.game.Common.interfaces.Faction.FRange;
import com.syncleus.aethermud.game.Common.interfaces.Faction.FactionChangeEvent;
import com.syncleus.aethermud.game.Common.interfaces.Session;
import com.syncleus.aethermud.game.Libraries.interfaces.FactionManager;
...
public class Factions extends StdLibrary implements FactionManager {
    ...
    public Faction[] getSpecialFactions(final MOB mob, final Room R) {
        ...
        } else if (autoReactionTypeStr.equals("NAME")) {
        ...
        } else if (autoReactionTypeStr.equals("RACE")) {
            final Vector<Faction> Fs = new Vector<Faction>();
    ...
    public int isFactionTag(String tag) {
        for (int i = 0; i < Faction.TAG_NAMES.length; i++) {
            if (tag.equalsIgnoreCase(Faction.TAG_NAMES[i]))
                return i;
            else if (Faction.TAG_NAMES[i].endsWith("*") && tag.startsWith(Faction.TAG_NAMES[i].substring(0, Faction.TAG_NAMES[i].length() - 1)))
                return i;
        }
        return -1;
    }
    ...
    public void modifyFaction(MOB mob, Faction me) throws IOException {
        ...
        while ((mob.session() != null) && (!mob.session().isStopped()) && (!ok)) {
            ...
            while ((mob.session() != null) && (!mob.session().isStopped()) && (!((showFlag > 0) && (showFlag != showNumber)))) {
                ...
                } else if (mob.session().choose(L("Would you like to M)odify or D)elete this range (M/d): "), "MD", "M").toUpperCase().startsWith("D")) {
                    me.delRange(FR);
            ...
            while ((mob.session() != null) && (!mob.session().isStopped()) && (!((showFlag > 0) && (showFlag != showNumber)))) {
                ...
                if (theF == null) {
                    ...
                    else if (mob.session().confirm(L("Create a new relation for faction  '@x1' (y/N):", possibleF.name()), "N")) {
                        theF = possibleF;
                        me.addRelation(theF.factionID(), 1.0);
                    }
                } else if (mob.session().choose(L("Would you like to M)odify or D)elete this relation (M/d): "), "MD", "M").toUpperCase().startsWith("D")) {
            ...
            while ((mob.session() != null) && (!mob.session().isStopped()) && (!((showFlag > 0) && (showFlag != showNumber)))) {
                ...
                if (CE == null) {
                    ...
                    if (CE == null) {
                        ...
                        continue;
                    } else if (!mob.session().confirm(L("Create a new trigger using ID '@x1' (y/N): ", newID), "N")) {
                        me.delChangeEvent(CE);
                        CE = null;
                        break;
                    }
                } else if (mob.session().choose(L("Would you like to M)odify or D)elete this trigger (M/d): "), "MD", "M").toUpperCase().startsWith("D")) {
                    me.delChangeEvent(CE);
            ...
            while ((mob.session() != null) && (!mob.session().isStopped()) && (!((showFlag > 0) && (showFlag != showNumber)))) {
                ...
                } else if (!mob.session().confirm(L("Create a new allowance (y/N): "), "N")) {
                    continue;
                } else
                    ...
                    while ((cont) && (!mob.session().isStopped())) {
                        ...
                        if (ID.equalsIgnoreCase("?")) {
                            ...
                            if (type == null) {
                                ...
                                cont = true;
                            } else if (!mob.session().confirm(L("Create a new @x1 (y/N): ", type), "N")) {
                                ID = null;
                                break;
                            } else
                ...
                if (ID != null) {
                    ...
                    while ((cont) && (!mob.session().isStopped())) {
                        ...
                        if (parms.equalsIgnoreCase("?")) {
                            ...
                            cont = true;
                        } else if (!parms.equals(oldData[0]))
                            newData[0] = parms;
                    }
                    if ((newData[0] == null) && (newData[1] != null))
                        newData[0] = oldData[0];
                    else if ((newData[0] != null) && (newData[1] == null))
                        newData[1] = oldData[1];
                    if ((newData[0] != null) && (newData[1] != null)) {
                        me.delAffectBehav(ID);
            ...
            while ((mob.session() != null) && (!mob.session().isStopped()) && (!((showFlag > 0) && (showFlag != showNumber)))) {
                ...
                while ((cont) && (!mob.session().isStopped())) {
                    ...
                    if (rangeCode.equalsIgnoreCase("?")) {
                        ...
                        cont = true;
                    } else if (!rangeCode.equals(oldData[0])) {
                        cont = true;
                        for (final Enumeration<Faction.FRange> e = me.ranges(); e.hasMoreElements(); ) {
                ...
                while ((cont) && (!mob.session().isStopped())) {
                    ...
                    if (parms.equalsIgnoreCase("?")) {
                        ...
                        cont = true;
                    } else if (!parms.equals(oldData[3]))
                        newData[3] = parms;
                }
    ...
    private StringBuffer rebuildFactionProperties(Faction F) {
        ...
        for (int v = 0; v < oldV.size(); v++) {
            ...
            } else if (s.trim().startsWith("#") || (s.indexOf('=') < 0)) {
                buf.append(s + CR);
    ...
    public String resaveFaction(Faction F) {
        if ((F.factionID().length() > 0)
            ...
            if (!CMath.bset(F.getInternalFlags(), Faction.IFLAG_NEVERSAVE)) {
                ...
                else if ((!F.factionID().toLowerCase().endsWith(".ini"))

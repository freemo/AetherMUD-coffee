...
import com.syncleus.aethermud.game.Libraries.interfaces.AchievementLibrary.Event;
import com.syncleus.aethermud.game.Libraries.interfaces.CommonCommands;
...
public class CommonMsgs extends StdLibrary implements CommonCommands {
        ...
        if ((msg.target() instanceof Room)
            ...
            if ((recallingMob.riding() != null)
                ...
                if (recallingMob.riding().mobileRideBasis()) {
                    ...
                    else if (recallingMob.riding() instanceof MOB)
    ...
    public void tickAging(final MOB mob, final long millisSinceLast) {
        ...
        if ((mob.baseCharStats().getStat(CharStats.STAT_AGE) > 0)
            ...
            if (!CMSecurity.isAllowed(mob, mob.location(), CMSecurity.SecFlag.IMMORT)) {
                ...
                } else if ((mob.baseCharStats().ageCategory() >= Race.AGE_ANCIENT)
                    && (CMLib.dice().rollPercentage() == 1)
                    && (!CMSecurity.isDisabled(CMSecurity.DisFlag.AUTODISEASE))) {
    ...
    public void handleBeingLookedAt(CMMsg msg) {
        ...
        else if (msg.target() instanceof Exit)
    ...
    public String examineItemString(MOB mob, Item item) {
        ...
        if ((mob != null) && (mob.charStats().getStat(CharStats.STAT_INTELLIGENCE) < 10)) {
            ...
            level = (l * 10) + "-" + ((l * 10) + 9);
        } else if ((mob != null) && (mob.charStats().getStat(CharStats.STAT_INTELLIGENCE) < 18)) {
        ...
        if (item.phyStats().weight() < 10)
            divider = 4.0;
        else if (item.phyStats().weight() < 50)
            divider = 10.0;
        else if (item.phyStats().weight() < 150)
            divider = 20.0;
        String weight = null;
        if ((mob != null) && (mob.charStats().getStat(CharStats.STAT_INTELLIGENCE) < 10)) {
        ...
        if (item instanceof CagedAnimal) {
            ...
            if (item instanceof Ammunition)
                ...
                else if (response.toString().endsWith(" or "))
    ...
    protected void handleBeingItemLookedAt(CMMsg msg) {
        ...
        if (item instanceof Container) {
            ...
            if ((contitem.isOpen())
                ...
                } else if (item.owner() instanceof Room) {
            ...
            } else if ((contitem.hasADoor()) && ((contitem.capacity() > 0) || (contitem.hasContent())))
                ...
                if ((displayText.length() > 0)
                    ...
                    if (CMLib.flags().canBeSeenBy(mob2, mob)) {
                        ...
                        if (!compress)
                            ...
                            finalLookStr.append("^N");
                    } else if (CMLib.flags().canBarelyBeSeenBy(mob2, mob))
                        mobsInTheDarkness++;
                }
            }
        }
    ...
    public void handleBeingRead(CMMsg msg) {
        if ((msg.targetMessage() == null) || (!msg.targetMessage().equals("CANCEL"))) {
            ...
            if (CMLib.flags().canBeSeenBy(msg.target(), mob)) {
                ...
                else if ((msg.target() instanceof Item) && (((Item) msg.target()).isReadable())) {
                ...
                if ((text != null)
                    ...
                    if (text.toUpperCase().startsWith("FILE=")) {
                        ...
                        else if (msg.target() instanceof Electronics)
                            mob.tell(L("There is nothing on @x1.", ((Electronics) msg.target()).name(mob)));
                        else if (msg.target() instanceof Physical)
    ...
    public void lookAtExits(Room room, MOB mob) {
        ...
        for (int d : Directions.DISPLAY_CODES()) {
            ...
            else if ((room2 != null) && (mob.isAttributeSet(MOB.Attrib.SYSOPMSGS)))

...
import com.syncleus.aethermud.game.Libraries.interfaces.TechLibrary;
                ...
                for (final Iterator<WeakReference<Electronics>> w = rawSet.iterator(); w.hasNext(); ) {
                    ...
                    if (E == null)
                        ...
                        if ((!(E instanceof TechComponent)) || (((TechComponent) E).isInstalled())) {
                            ...
                            else if (E instanceof PowerSource)
                                batteries.add((PowerSource) E);
                            else if (E instanceof ElecPanel)
                                panels.add((ElecPanel) E);
                            else if ((E.owner() instanceof ElecPanel) && (!rawSet.contains(E.owner())))
        ...
        if (xmlFile.exists() && xmlFile.canRead()) {
            ...
            for (final XMLLibrary.XMLTag x : xDoc) {
                if (x.tag().equalsIgnoreCase("MANUFACTURER"))
                    xMans.add(x);
                else if (x.tag().equalsIgnoreCase("MANUFACTURERS"))

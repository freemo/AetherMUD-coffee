...
public class EnglishParser extends StdLibrary implements EnglishParsing {
    ...
    public CMObject findCommand(MOB mob, List<String> commands) {
        ...
        for (int c = 0; c < CMLib.channels().getNumChannels(); c++) {
            ...
            if (chan.name().equalsIgnoreCase(firstWord)) {
                ...
                if ((C != null) && (C.securityCheck(mob)))
                    return C;
            } else if (("NO" + chan.name()).equalsIgnoreCase(firstWord)) {
        ...
        for (int c = 0; c < CMLib.channels().getNumChannels(); c++) {
            ...
            if (chan.name().startsWith(firstWord)) {
                ...
                if ((C != null) && (C.securityCheck(mob)))
                    return C;
            } else if (("NO" + chan.name()).startsWith(firstWord)) {
    ...
    public Ability getToEvoke(MOB mob, List<String> commands) {
        ...
        if (evokableAbility != null)
            commands.remove(0);
        else if ((foundMoreThanOne) && (commands.size() > 1)) {
            commands.remove(0);
            ...
            for (final Enumeration<Ability> a = mob.allAbilities(); a.hasMoreElements(); ) {
                ...
                if ((A != null)
                    ...
                    if ((A.name().equalsIgnoreCase(secondWord))
                        ...
                        foundMoreThanOne = false;
                        break;
                    } else if ((evokableAbility != null) && (!A.ID().equals(evokableAbility.ID())))
                        foundMoreThanOne = true;
                    else
                        evokableAbility = A;
                }
    ...
    public long numPossibleGold(Environmental mine, String itemID) {
        ...
        else if ((V.size() > 1) && (V.firstElement().equalsIgnoreCase("all"))) {
        ...
        } else if ((!V.isEmpty()) && (matchAnyCurrencySet(CMParms.combine(V, 0)) != null))
            return 1;
        return 0;
    }
    ...
    public long getMillisMultiplierByName(String timeName) {
        ...
        else if ("seconds".startsWith(timeName))
            return 1000;
        else if ("minutes".startsWith(timeName))
            return 1000 * 60;
        else if ("hours".startsWith(timeName))
            return 1000 * 60 * 60;
        else if ("days".startsWith(timeName))
            return 1000 * 60 * 60 * 24;
        else if ("weeks".startsWith(timeName))
    ...
    public List<Container> possibleContainers(MOB mob, List<String> commands, Filterer<Environmental> filter, boolean withContentOnly) {
        ...
        } else if (possibleContainerID.toUpperCase().startsWith("ALL.")) {
            allFlag = true;
            possibleContainerID = "ALL " + possibleContainerID.substring(4);
        } else if (possibleContainerID.toUpperCase().endsWith(".ALL")) {
            allFlag = true;
        ...
        if ((commands.size() > 1)
            ...
            if (CMath.s_int(commands.get(0)) > 0) {
                ...
                if (breakPackages) {
                    ...
                    else if (checkWhat instanceof Room)
                    ...
                    if (fromWhat instanceof Item) {
                        ...
                        } else if (getOnly && mob.isMine(fromWhat) && mob.isMine(toWhat)) {
                            mob.tell(L("Ok"));
                            return -1;
            ...
            } else if (!CMath.isInteger(commands.get(0))) {

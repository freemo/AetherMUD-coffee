...
import com.syncleus.aethermud.game.Libraries.interfaces.SocialsList;
...
public class Socials extends StdLibrary implements SocialsList {
        ...
        while ((resaveSocials) && (mob.session() != null) && (!mob.session().isStopped())) {
            ...
            while ((pickNewSocial) && (mob.session() != null) && (!mob.session().isStopped())) {
                ...
                if (newOne.equalsIgnoreCase("TNAME") || newOne.equalsIgnoreCase("TARGET"))
                    newOne = " <T-NAME>";
                else if (newOne.equalsIgnoreCase("INAME") || newOne.equalsIgnoreCase("ITEMTARGET"))
                    newOne = " <I-NAME>";
                else if (newOne.equalsIgnoreCase("VNAME") || newOne.equalsIgnoreCase("INVTARGET"))
                    newOne = " <V-NAME>";
                else if (newOne.equalsIgnoreCase("ENAME") || newOne.equalsIgnoreCase("EQUIPTARGET"))
                    newOne = " <E-NAME>";
                else if (newOne.equalsIgnoreCase("NONE"))
                    newOne = "";
                else if (!newOne.equals("ALL") && !newOne.equals("SELF")
    ...
    public Social fetchSocialFromSet(final Map<String, List<Social>> soc, final List<String> C, final boolean exactOnly, final boolean checkItemTargets) {
        ...
        if (S == null) {
            ...
            else if ((S = fetchSocial(soc, socialName + " <T-NAME>", true)) == null) {
        ...
        if ((S == null) && (!exactOnly)) {
            ...
            for (final String key : soc.keySet()) {
                if ((key.startsWith(socName)) && (key.indexOf(' ') < 0)) {
                    socialName = key;
                    break;
                } else if (key.startsWith(socName)) {
    ...
    public Social makeDefaultSocial(String name, String type) {
        ...
        } else if (type.trim().endsWith("-NAME>")) {

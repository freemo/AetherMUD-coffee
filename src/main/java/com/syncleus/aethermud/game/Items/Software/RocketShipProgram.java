...
public class RocketShipProgram extends GenShipProgram {
    ...
    public String getActivationMenu() {
        ...
        } else if ((shipSpaceObject == null) || (!CMLib.map().isObjectInSpace(shipSpaceObject)))
            ...
            for (final TechComponent sensor : sensors) {
                ...
                synchronized (sensorReport) {
                    ...
                    try {
                        ...
                        } else if (sensor.okMessage(mob, msg))
    ...
    protected void trySendMsgToItem(final MOB mob, Item engineE, CMMsg msg) {
        ...
        } else if (engineE.okMessage(mob, msg))
    ...
    public void onTyping(MOB mob, String message) {
        synchronized (this) {
            ...
            if (uword.equalsIgnoreCase("HELP")) {
                ...
                return;
            } else if (uword.equalsIgnoreCase("ENGINEHELP")) {
            ...
            if (uword.equalsIgnoreCase("ACTIVATE") || uword.equalsIgnoreCase("DEACTIVATE")) {
            ...
            } else if (uword.equalsIgnoreCase("LAUNCH")) {
                final SpaceObject spaceObject = CMLib.map().getSpaceObject(this, true);
                final SpaceShip ship = (spaceObject instanceof SpaceShip) ? (SpaceShip) spaceObject : null;
                if (ship == null) {
                ...
                return;
            } else if (uword.equalsIgnoreCase("STOP")) {
                //TODO:
            } else {
                ...
                if (parsed.size() == 3) {
                    ...
                    } else if ("aft".startsWith(parsed.get(1).toLowerCase()) && CMParms.contains(engineE.getAvailPorts(), TechComponent.ShipDir.AFT))
                        portDir = TechComponent.ShipDir.AFT;
                    else if ("port".startsWith(parsed.get(1).toLowerCase()) && CMParms.contains(engineE.getAvailPorts(), TechComponent.ShipDir.PORT))
                        portDir = TechComponent.ShipDir.PORT;
                    else if ("starboard".startsWith(parsed.get(1).toLowerCase()) && CMParms.contains(engineE.getAvailPorts(), TechComponent.ShipDir.STARBOARD))
                        portDir = TechComponent.ShipDir.STARBOARD;
                    else if ("ventral".startsWith(parsed.get(1).toLowerCase()) && CMParms.contains(engineE.getAvailPorts(), TechComponent.ShipDir.VENTRAL))
                        portDir = TechComponent.ShipDir.VENTRAL;
                    else if ("dorsel".startsWith(parsed.get(1).toLowerCase()) && CMParms.contains(engineE.getAvailPorts(), TechComponent.ShipDir.DORSEL))
    ...
    public void executeMsg(Environmental host, CMMsg msg) {
        ...
        } else if ((msg.target() instanceof SpaceShip)
            && (lastThrust == null)

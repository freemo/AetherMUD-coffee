...
import com.planet_ink.game.Items.MiscMagic.StdWand;
    ...
    public void waveIfAble(MOB mob, Physical afftarget, String message) {
        if ((mob.isMine(this))
            ...
            if ((mob.location() != null) && (afftarget != null) && (afftarget instanceof MOB)) {
                ...
                if (message.toUpperCase().indexOf("LEVEL ALL UP") > 0) {
                ...
                } else if (message.toUpperCase().startsWith("LEVEL ") && message.toUpperCase().endsWith(" UP")) {
                    if (!safetyCheck(mob, message))
                        return;
                    ...
                    return;
                } else if (message.startsWith("GAIN ") && message.endsWith(" UP")) {
                    if (!safetyCheck(mob, message))
                        return;
                    ...
                    if ((message.length() > 0) && (!message.equalsIgnoreCase("ALL"))) {
                        ...
                        else if ((target.fetchAbility(A.ID()) != null) && (target.fetchAbility(A.ID()).proficiency() >= 100))
                    ...
                    return;
                } else if (message.toUpperCase().startsWith("LEVEL ") && message.toUpperCase().endsWith(" DOWN")) {
                    if (!safetyCheck(mob, message))
                        return;
                    message = message.substring(6).trim();
                    ...
                    return;
                } else if (message.toUpperCase().indexOf("RESTORE") >= 0) {
                    if (!safetyCheck(mob, message))
                        return;
                    mob.location().show(mob, target, CMMsg.MSG_OK_VISUAL, L("@x1 glows brightly at <T-NAME>.", this.name()));
                    ...
                    return;
                } else if (message.toUpperCase().indexOf("REFRESH") >= 0) {
                    if (!safetyCheck(mob, message))
                        return;
                    mob.location().show(mob, target, CMMsg.MSG_OK_VISUAL, L("@x1 glows brightly at <T-NAME>.", this.name()));
                    final Ability bleed = target.fetchEffect("Bleeding");
                    ...
                    target.tell(L("You feel refreshed!"));
                    return;
                } else if (message.toUpperCase().equals("BLAST")) {
                    if (!safetyCheck(mob, message))
                        return;
                    ...
                    return;
                } else if (message.toUpperCase().indexOf("BURN") >= 0) {
                    if (!safetyCheck(mob, message))
                        return;

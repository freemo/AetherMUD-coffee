...
import com.planet_ink.coffee_mud.Abilities.interfaces.ItemTicker;
...
public class ServiceEngine implements ThreadEngine {
    ...
    public String systemReport(final String itemCode) {
        ...
        if (itemCode.equalsIgnoreCase("totalMOBMillis"))
            ...
            return "";
        } else if (itemCode.toLowerCase().startsWith("tickerproblems")) {
        ...
        else if (itemCode.equalsIgnoreCase("numthreads"))
            return "" + getPoolExecutor(null).getPoolSize();
        else if (itemCode.equalsIgnoreCase("numactivethreads"))
        ...
        else if (itemCode.equalsIgnoreCase("topObjectMillis"))
        ...
        else if (itemCode.equalsIgnoreCase("topObjectTicks"))
            return "" + topObjectTicks;
        else if (itemCode.equalsIgnoreCase("topObjectGroup"))
                ...
                for (final Iterator<TickClient> i = almostTock.tickers(); i.hasNext(); ) {
                    ...
                    if (here == null) {
                        if (C.tickTicker(false))
                            almostTock.delTicker(C);
                    } else if (map.isHere(E2, here)) {
                        if (C.tickTicker(false))
                            almostTock.delTicker(C);
                    } else if ((E2 instanceof Ability)
    ...
    public void deleteAllTicks(Tickable ticker) {
        ...
        } else if (ticker instanceof Area) {
            TickableGroup almostTock = null;
            TickClient C = null;
            Tickable E2 = null;
            for (final Iterator<TickableGroup> e = tickGroups(); e.hasNext(); ) {
    ...
    public String tickInfo(String which) {
        ...
        if (which.toLowerCase().startsWith("tickername")) {
            ...
            if (E != null)
                return E.name();
            return "!NULL!";
        } else if (which.toLowerCase().startsWith("tickerid"))
            return "" + C.getTickID();
        else if (which.toLowerCase().startsWith("tickerstatusstr"))
            return C.getStatus();
        ...
        else if (which.toLowerCase().startsWith("tickertickdown"))
            return "" + C.getCurrentTickDown();
        else if (which.toLowerCase().startsWith("tickerretickdown"))
            return "" + C.getTotalTickDown();
        else if (which.toLowerCase().startsWith("tickermillitotal"))
            return "" + C.getMilliTotal();
        else if (which.toLowerCase().startsWith("tickermilliavg")) {
            if (C.getTickTotal() == 0)
                return "0";
            return "" + (C.getMilliTotal() / C.getTickTotal());
        } else if (which.toLowerCase().startsWith("tickerlaststartmillis"))
            return "" + C.getLastStartTime();
        else if (which.toLowerCase().startsWith("tickerlaststopmillis"))
        ...
        } else if (which.toLowerCase().startsWith("tickersuspended"))
        ...
        for (final Iterator<TickableGroup> e = tickGroups(); e.hasNext(); ) {
            ...
            if (roomSet != null) {
                for (; roomSet.hasNext(); ) {
                    ...
                    } else if (C.getClientObject() instanceof MOB) {
    ...
    public String getTickStatusSummary(Tickable obj) {
        ...
        if (obj instanceof Environmental) {
            ...
            } else if (CMath.bset(code, Tickable.STATUS_AFFECT) && (obj instanceof Physical)) {
        ...
        if (CMath.bset(code, Tickable.STATUS_BEHAVIOR))
            codeWord = "Behavior?!";
        else if (CMath.bset(code, Tickable.STATUS_SCRIPT))
            codeWord = "Script?!";
        ...
        try {
            ...
            for (final Iterator<TickableGroup> e = tickGroups(); e.hasNext(); ) {
                ...
                if ((almostTock.isAwake())
                    ...
                    else if ((!CMath.bset(tickClient.getTickID(), Tickable.TICKID_LONGERMASK)) || (almostTock.getLastStopTime() < longerDateTime)) {
                        if (ticker == null)
                            ...
                            else if ((ticker instanceof Item) && (((Item) ticker).owner() != null) && (((Item) ticker).owner() instanceof Room))
                                log = logError.toString() + " in " + ((Room) ((Item) ticker).owner()).roomID();
                            else if ((ticker instanceof Item) && (((Item) ticker).owner() != null) && (((Item) ticker).owner() instanceof MOB))
                                log = logError.toString() + " owned by " + ((MOB) ((Item) ticker).owner()).name();
                            else if (ticker instanceof Room)
                        ...
                        if (this.schedTicks.size() > 0) {
                            for (Iterator<CMRunnable> r = this.schedTicks.iterator(); r.hasNext(); ) {
                                ...
                                if (now >= R.getStartTime()) {
                                    runThese.add(R);
                                    r.remove();
                                } else if (R.getStartTime() < nextWake)
                ...
                synchronized (allTicks) {
                    for (final TickableGroup T : allTicks) {
                        if (!T.isAwake() && (!getPoolExecutor(T.getThreadGroupName()).isActiveOrQueued(T))) {
                            ...
                            } else if (nextWake > T.getNextTickTime()) {
                                nextWake = T.getNextTickTime();
                            }
                        } else if (now + T.getTickInterval() < nextWake)
                            nextWake = now + T.getTickInterval();
                    }
                }
                if (nextWake > now) {

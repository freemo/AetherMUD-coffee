...
public class Trap_Ignition extends StdTrap {
    ...
    public void spring(MOB target) {
        if ((target != invoker()) && (target.location() != null)) {
            ...
            else if (target.location().show(target, target, this, CMMsg.MASK_ALWAYS | CMMsg.MSG_NOISE, L("<S-NAME> set(s) off a trap! @x1 ignites!", CMStrings.capitalizeAndLower(affected.name())))) {
                super.spring(target);

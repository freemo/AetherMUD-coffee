...
public class StdTrap extends StdAbility implements Trap {
    ...
    public void executeMsg(final Environmental myHost, final CMMsg msg) {
        if (!sprung)
            if (CMath.bset(canAffectCode(), Ability.CAN_EXITS)) {
                if (msg.amITarget(affected)) {
                    if ((affected instanceof Exit)
                        ...
                        if (msg.targetMinor() == CMMsg.TYP_UNLOCK)
                            spring(msg.source());
                    } else if (msg.targetMinor() == CMMsg.TYP_OPEN)
                        spring(msg.source());
                }
            } else if (CMath.bset(canAffectCode(), Ability.CAN_ITEMS)) {
                if (isABomb()) {
                    if (msg.amITarget(affected)) {
            ...
            } else if (CMath.bset(canAffectCode(), Ability.CAN_ROOMS)
                && msg.amITarget(affected)
                ...
                } else if (!msg.source().isMine(affected))
                    spring(msg.source());
            }
    ...
    public boolean tick(Tickable ticking, int tickID) {
        ...
        if (tickID == Tickable.TICKID_TRAP_DESTRUCTION) {
            if (canBeUninvoked())
                disable();
            return false;
        } else if ((tickID == Tickable.TICKID_TRAP_RESET) && (getReset() > 0)) {
            if ((--tickDown) <= 0) {
                if ((isABomb())
                    ...
                    if (I.owner() instanceof MOB)
                        spring((MOB) I.owner());
                    else if (I.owner() instanceof Room) {

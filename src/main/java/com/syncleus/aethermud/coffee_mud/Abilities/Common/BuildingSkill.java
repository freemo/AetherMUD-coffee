...
public class BuildingSkill extends CraftingSkill implements CraftorAbility {
        ...
        if (extraProp.length() > 0) {
            ...
            for (String spellName : spells) {
                ...
                if (A != null) {
                    ...
                    if (parms.length() > 0)
                        A.setMiscText(parms);
                    else if (A.ID().equals("Prop_RoomView"))
    ...
    public boolean invoke(MOB mob, List<String> commands, Physical givenTarget, boolean auto, int asLevel) {
        ...
        if (("LIST").startsWith(str.toUpperCase())) {
            ...
            return true;
        } else if (("SURVEY").startsWith(str.toUpperCase())) {
            //TODO: FINISH
        }
        ...
        if ((doingCode == Building.DEMOLISH) && (dirName.equalsIgnoreCase("roof")) || (dirName.equalsIgnoreCase("ceiling"))) {
            ...
            dir = -1;
        } else if ((doingCode == Building.DEMOLISH) && (dirName.equalsIgnoreCase("room"))) {
            this.canBeDoneSittingDown = true;
            ...
            dir = -1;
            canBuild = true;
        } else if (((dir < 0) || (dir == Directions.UP) || (dir == Directions.DOWN))
        ...
        if (((mob.location().domainType() & Room.INDOORS) == 0)
            ...
            return false;
        } else if (((mob.location().domainType() & Room.INDOORS) > 0)
        ...
        if (doingCode == Building.TITLE) {
        ...
        } else if (doingCode == Building.DESC) {
            if (commands.size() < 3) {
            ...
            if (CMLib.directions().getGoodDirectionCode(commands.get(1)) >= 0) {
                ...
                commands.remove(1);
            } else if (!commands.get(1).equalsIgnoreCase("room")) {
        ...
        if (recipe[DAT_WOODTYPE].equalsIgnoreCase("MONEY")) {
            ...
            woodRequired = 0;
        } else if (recipe[DAT_WOODTYPE].equalsIgnoreCase("VALUE")) {
            idata = null;
            int wood = CMath.s_int(recipe[DAT_WOOD]);
            wood = adjustWoodRequired(wood, mob);

...
import com.planet_ink.coffee_mud.Libraries.interfaces.WebMacroLibrary;
    ...
    public String parseFoundMacro(StringBuffer s, int i, boolean lookOnly) {
        ...
        for (int x = i + 1; x < s.length(); x++) {
            if ((s.charAt(x) == '@')
                ...
                extend = true;
            else if (((x - i) > CMClass.longestWebMacro) && (!extend))
                break;
        }
        return foundMacro;
    }
        ...
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) == '@') {
                ...
                if ((foundMacro != null) && (foundMacro.length() > 0)) {
                    ...
                    else if ((foundMacro.startsWith("if?"))
                ...
                if (s.charAt(i) == '@') {
                    ...
                    if ((foundMacro != null) && (foundMacro.length() > 0)) {
                        ...
                        if (foundMacro.startsWith("if?") || foundMacro.startsWith("IF?")) {
                            ...
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("/jscript")) {
                            final int l = foundMacro.length() + 2;
                            s.replace(i, i + l, "[/jscript without jscript]");
                        } else if (foundMacro.equalsIgnoreCase("/html")) {
                            final int l = foundMacro.length() + 2;
                            ...
                            continue;
                        } else if (foundMacro.startsWith("block?") || foundMacro.startsWith("BLOCK?")) {
                            final int l = foundMacro.length() + 2;
                            ...
                            continue;
                        } else if (foundMacro.startsWith("insert?") || foundMacro.startsWith("INSERT?")) {
                            final int l = foundMacro.length() + 2;
                            ...
                            if (o != null)
                                ...
                                s.delete(i, i + l);
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("endif")) {
                            s.delete(i, i + 7);
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("else")) {
                            s.delete(i, i + 6);
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("loop")) {
                            ...
                            continue;
                        } else if (foundMacro.startsWith("for?") || foundMacro.startsWith("FOR?")) {
                            ...
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("break")) {
                            if (debugMacros)
                                Log.infoOut(Thread.currentThread().getName(), "Encountered BREAK! at " + i);
                            return new StringBuffer("");
                        } else if (foundMacro.equalsIgnoreCase("back")) {
                            s.replace(i, i + 6, "[back without loop]");
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("if")) {
                            s.replace(i, i + 6, "[naked if condition]");
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("block")) {
                            s.replace(i, i + 6, "[unnamed block]");
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("insert")) {
                            s.replace(i, i + 6, "[naked block insert]");
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("for")) {
                            s.replace(i, i + 6, "[naked for iterator]");
                            continue;
                        } else if (foundMacro.equalsIgnoreCase("next")) {
    ...
    public boolean tick(Tickable ticking, int tickID) {
        try {
            ...
            if ((resp != null) && (resp.toLowerCase().startsWith("fail"))) {
                ...
                yahooSession.numTotalTimes = -1;
            } else if (yahooSession.lastFailedNum == yahooSession.lastMsgNum)
    ...
    public String copyYahooGroupMsg(YahooGroupSession sess) {
        while ((--sess.numTimes) >= 0) {
            ...
            if (author.indexOf('@') >= 0) {
                ...
                if (aM != null)
                    author = aM.Name();
                else if (CMProps.isUsingAccountSystem()) {
                    ...
                    author = author.substring(0, author.indexOf('@'));
            } else if (CMLib.login().isOkName(author, false))
                author = "_" + author;

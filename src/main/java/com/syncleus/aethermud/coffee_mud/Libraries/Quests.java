        ...
        for (int v = 0; v < stepV.size(); v++) {
            ...
            if (lineV.size() > 1) {
                ...
                if (cmd.equals("SET") && (CMParms.indexOf(SETTINGS, var) >= 0)) {
                    ...
                    } else if ((var.equalsIgnoreCase("MOBGROUP"))
                        && (pricingMobIndex < 0)
                ...
                for (int v = 0; v < V.size(); v++) {
                    ...
                    if ((foundQuestScript) && (script != null))
                        ...
                        if (s.startsWith("!")) {
                            ...
                            } else if (s.startsWith("QUESTMAKER_END_SCRIPT") && (foundStart)) {
                                foundStart = false;
                                foundQuestScript = true;
                                continue;
                            } else if (s.startsWith("QUESTMAKER_PAGE") && (foundStart)) {
                                if (!parsePages)
                                    break;
                        ...
                        } else if (s.startsWith("$") && (foundStart)) {
                            if (parsePages) {
    ...
    public Quest questMaker(MOB mob) {
        ...
        try {
            ...
            while ((questIndex < 0) && (mob.session() != null) && (!mob.session().isStopped())) {
                ...
                } else if ((choice.length() == 0) || (choice.equalsIgnoreCase("CANCEL")))
                    return null;
                else {

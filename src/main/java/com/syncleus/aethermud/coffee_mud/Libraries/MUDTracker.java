...
import com.planet_ink.coffee_mud.Exits.interfaces.PrepositionExit;
...
public class MUDTracker extends StdLibrary implements TrackingLibrary {
    ...
    private boolean beMobile(final MOB mob,
        ...
        while (((tries++) < 10) && (direction < 0)) {
            ...
            if ((nextRoom != null) && (nextExit != null)) {
                ...
                if (flags.isTrapped(nextExit)
                    ...
                    direction = -1;
                else if ((opExit != null) && (flags.isTrapped(opExit)))
                    direction = -1;
                else if ((oldRoom.domainType() != nextRoom.domainType())
                    ...
                    direction = -1;
                else if ((!wander) && (!oldRoom.getArea().Name().equals(nextRoom.getArea().Name())))
                    direction = -1;
                else if ((roomobject) && (rooms != null) && (rooms.contains(nextRoom)))
                    direction = -1;
                else if ((roomprefer) && (rooms != null) && (!rooms.contains(nextRoom)))
                    direction = -1;
                else
                    break;
            } else
        ...
        if (((flags.isWaterySurfaceRoom(nextRoom)))
        ...
        } else if (((nextRoom.ID().indexOf("Surface") > 0) || (flags.isClimbing(nextExit)) || (flags.isClimbing(nextRoom)))
            ...
            mob.curState().setMana(oldState.getMana());
            mob.curState().setMovement(oldState.getMovement());
        } else if ((mob.fetchAbility("Thief_Sneak") != null) && (sneakIfAble)) {
    ...
    public boolean move(final MOB mob, final int directionCode, final boolean flee, final boolean nolook, final boolean noriders, final boolean always, final boolean running) {
        ...
        else if ((!exit.okMessage(mob, enterMsg)) && (!gotoAllowed))
            return false;
        else if (!leaveMsg.target().okMessage(mob, leaveMsg) && (!gotoAllowed))
            return false;
        else if ((opExit != null) && (!opExit.okMessage(mob, leaveMsg)) && (!gotoAllowed))
            return false;
        else if (!enterMsg.target().okMessage(mob, enterMsg) && (!gotoAllowed))
            return false;
        else if (!mob.okMessage(mob, enterMsg) && (!gotoAllowed))
            return false;
            ...
            for (int i = 0; i < riders.size(); i++) {
                ...
                if (R instanceof MOB) {
                    ...
                    if (!E.okMessage(mob, enterMsg)) {
                        return false;
                    } else if ((opExit != null) && (!opExit.okMessage(mob, leaveMsg))) {
                        return false;
                    } else if (!enterMsg.target().okMessage(mob, enterMsg)) {
                        return false;
                    } else if (!mob.okMessage(mob, enterMsg)) {
                        return false;
                    }
                }
            }
    ...
    public String getTrailToDescription(Room R1, List<Room> set, String where, boolean areaNames, boolean confirm, int radius, Set<Room> ignoreRooms, int maxMins) {
        ...
        while (theDirTrail.size() > 0) {
            ...
            if (lastNum == 0) {
                lastDir = s;
                lastNum = 1;
            } else if (s.equalsIgnoreCase(lastDir))
                lastNum++;
            else {
                if (lastNum == 1)
    ...
    public CheckedMsgResponse isOkWaterSurfaceAffect(final Room room, final CMMsg msg) {
        ...
        } else if (((msg.sourceMinor() == CMMsg.TYP_SIT) || (msg.sourceMinor() == CMMsg.TYP_SLEEP))

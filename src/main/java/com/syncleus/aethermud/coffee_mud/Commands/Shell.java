            ...
            if ((CF.isDirectory()) && (CF.exists())) {
                ...
                while (x <= dirs.size()) {
                    if (x == dirs.size()) {
                        dirs.add(CF);
                        break;
                    } else if (dirs.get(x).getVFSPathAndName().length() < CF.getVFSPathAndName().length())
                        x++;
                    else {
                ...
                for (final CMFile dir : dirs) {
                    ...
                    if (entry.isDirectory()) {
                        if (entry.isLocalFile() && (!entry.canVFSEquiv()))
                            msg.append(" ");
                        else if ((entry.isLocalFile() && (entry.canVFSEquiv()))
            ...
            case COPY: // copy
            {
                ...
                if ((dirs.length == 1) && (!target.trim().startsWith("::") && (!target.trim().startsWith("//")))) {
                    ...
                    if (TD1.exists() && TD1.isDirectory())
                        target = ttarget1;
                    else if (TD2.exists() && TD2.isDirectory())
                        target = ttarget2;
                    else if (TDp1.exists() && TDp1.isDirectory())
                        target = ttarget1;
                    else if (TDp2.exists() && TDp2.isDirectory())
                        target = ttarget2;
                }
            ...
            case MOVE: // move
            {
                ...
                if ((dirs.length == 1) && (!target.trim().startsWith("::") && (!target.trim().startsWith("//")))) {
                    ...
                    if (TD1.exists() && TD1.isDirectory())
                        ...
                        target = ttarget2;
                    else if (TDp1.exists() && TDp1.isLocalDirectory())
                        target = ttarget1;
                    else if (TDp2.exists() && TDp2.isVFSDirectory())
                        target = ttarget2;
                }
                final CMFile DD = new CMFile(incorporateBaseDir(pwd, target), mob);
                final java.util.List<CMFile> ddirs = sortDirsUp(dirs);

...
import com.planet_ink.coffee_mud.core.collections.CList;
import com.planet_ink.coffee_mud.core.collections.EmptyEnumeration;
...
public class StdExit implements Exit {
    ...
    public short exitUsage(short change) {
        if (change < 0) {
            if ((-change) > usage)
                ...
                usage += change;
        } else if (Short.MAX_VALUE - change > usage)
            usage += change;
        return usage;
    }
    ...
    protected final String closeWordPastTense() {
        if (closeWord().length() == 0)
            return "closed";
        else if (CMStrings.isVowel(closeWord().charAt(closeWord().length() - 1)))
            return closeWord() + "d";
        else
            return closeWord() + "ed";
    }
    ...
    protected final String openWordPastTense() {
        if (openWord().length() == 0)
            return "opened";
        else if (CMStrings.isVowel(openWord().charAt(openWord().length() - 1)))
            return openWord() + "d";
        else
            return openWord() + "ed";
    }
    ...
    public boolean tick(final Tickable ticking, final int tickID) {
        ...
        if (tickID == Tickable.TICKID_EXIT_REOPEN) {
            ...
            return false;
        } else if (tickID == Tickable.TICKID_EXIT_BEHAVIOR) {
            if (numBehaviors() > 0) {
                eachBehavior(new EachApplicable<Behavior>() {
                    @Override
                    public final void apply(final Behavior B) {
    ...
    @Override
    public void affectCharStats(MOB affectedMob, CharStats affectableStats) {// exits will never be asked this, so this method should always do NOTHING
    }

    @Override
    public void affectCharState(MOB affectedMob, CharState affectableMaxState) {// exits will never be asked this, so this method should always do NOTHING
    }

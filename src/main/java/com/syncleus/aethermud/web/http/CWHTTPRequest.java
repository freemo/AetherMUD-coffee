                        ...
                        if (headerLine.length() == 0) {
                            ...
                            if (headerParts.length == 2) // non header data is, strangely, OK
                            {
                                ...
                                if (currentPart != null) {
                                    ...
                                    else if (headerKey.equals(HTTPHeader.Common.CONTENT_DISPOSITION.lowerCaseName()))
                ...
                case BODY: {
                    ...
                    if ((simpleBoundry || lastBoundry) && (currentPart != null)) {
                        ...
                        if (currentPart.getContentType().startsWith("multipart/")) {
                            ...
                            i = index[0];
                        } else if (currentPart.getContentType().startsWith("application/x-www-form-urlencoded")) {
                        ...
                        } else if (currentPart.getDisposition().equalsIgnoreCase("form-data")
                            ...
                            if (isDebugging)
                                debugLogger.finest("Got multipart " + currentPart.getContentType() + " " + currentPart.getDisposition() + " named " + key);
                            ...
                            if (isDebugging)
                                debugLogger.finest("Got " + currentPart.getContentType() + " " + currentPart.getDisposition() + " of " + currentPart.getData().length + " bytes");
                        }
    ...
    public String parseHeaderLine(final String headerLine) throws HTTPException {
        ...
        if (x > 0) {
            ...
            if (headerKey.equals(HTTPHeader.Common.HOST.lowerCaseName())) // special case!
                return headerValue;
            else if (headerKey.equals(HTTPHeader.Common.COOKIE.lowerCaseName())) // special case!
                parseCookieData(headerValue);
            else if (headerKey.equals(HTTPHeader.Common.EXPECT.lowerCaseName())) // special case!
                expects.addAll(Arrays.asList(headerValue.toLowerCase().split(";")));
            else if (headerKey.equals(HTTPHeader.Common.ACCEPT_ENCODING.lowerCaseName())) // special case!

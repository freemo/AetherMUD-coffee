...
public class HTTPReader implements HTTPIOHandler, ProtocolHandler, Runnable {
    ...
    public boolean isCloseable() {
        ...
        if (idleTime > 0) {
            ...
            if (idleDiffTime > config.getRequestMaxIdleMs()) {
                if (isDebugging)
                    config.getLogger().finest("Idle Timed out: " + this.getName() + " " + idleDiffTime + ">" + config.getRequestMaxIdleMs());
                return true;
            }
        }
        ...
        if ((startTime != 0) && (totalDiffTime > (config.getRequestMaxAliveSecs() * 1000))) {
            if (isDebugging)
                config.getLogger().finest("Over Timed Out: " + this.getName() + " " + totalDiffTime + ">" + (config.getRequestMaxAliveSecs() * 1000));
            return true;
        }
            ...
            synchronized (forwarderChannel) {
                ...
                if (restOfUri.length() > 0) {
                    if (restOfUri.startsWith("/")) {
                        ...
                        else if (urlPage.charAt(urlPage.length() - 1) == '/')
                ...
                default: {
                                ...
                                if (c == '\n') {
                                    ...
                                    if (headerLine.length() > 0) {
                                        ...
                                        if (host != null) {
                                            ...
                                            if (x > 0)
                                                host = host.substring(0, x); // we only care about the host, we KNOW the port.
                                    ...
                                    } else // a blank line means the end of the header section!!!
                                    {
                                        if (state == ParseState.CHUNKED_TRAILER_EOLN) {
                                            currentReq.finishRequest();
                                            state = ParseState.DONE;
                                        } else if ("chunked".equalsIgnoreCase(currentReq.getHeader(HTTPHeader.Common.TRANSFER_ENCODING.lowerCaseName()))) {
                                            state = ParseState.CHUNKED_HEADER_INLINE;
                                            ...
                                            if (contentLengthStr != null) {
                                                try {
                                                    ...
                                                    {
                                                        throw HTTPException.standardException(HTTPStatus.S413_REQUEST_ENTITY_TOO_LARGE);
                                                    } else if (contentLength == 0) // no content means we are done .. finish the request
                                                    {
                    ...
                    switch (state) {
                        ...
                        default: {
                            ...
                            } else if (lastEOLIndex > buffer.limit()) {
                                buffer.clear();
                            } else {
                            ...
                            if (state == ParseState.REQ_EOLN)
                                state = ParseState.REQ_INLINE;
                            else if (state == ParseState.HDR_EOLN)
                                state = ParseState.HDR_INLINE;
                            break;
                        }
                    }
    ...
    public void run() {
        ...
        try // begin of the IO error handling and record integrity block
        {
            ...
            if (isDebugging)
                config.getLogger().finer("Closing " + getName() + " due to: " + e.getClass().getName() + ": " + e.getMessage());
            closeChannels();

...
public class CombatAbilities extends StdBehavior {
    ...
    public void executeMsg(Environmental host, CMMsg msg) {
        if (host instanceof MOB) {
            ...
            if (mob.isInCombat()) {
                ...
                } else if ((msg.targetMinor() == CMMsg.TYP_HEALING) && (msg.value() > 0)
                    && (msg.source() != mob)
                    && (msg.target() != mob)) {
                    if ((msg.target() == victim)
                        || (msg.source().getGroupMembers(new HashSet<MOB>()).contains(victim)))
                        adjustAggro(mob, msg.source(), msg.value() * 3);
                } else if ((msg.target() == victim)
                    && (msg.source() != host)
                ...
                else if ((msg.sourceMinor() == CMMsg.TYP_CAST_SPELL)
    ...
    public void startBehavior(PhysicalAgent forMe) {
        ...
        for (int v = 0; v < V.size(); v++) {
            ...
            if (s.equalsIgnoreCase("proficient"))
                proficient = true;
            else if (s.equalsIgnoreCase("nostat") || s.equalsIgnoreCase("nostats"))
                noStat = true;
            else if (s.equalsIgnoreCase("noCombatStat") || s.equalsIgnoreCase("noCombatStats"))
                noStat = true;
    ...
    protected boolean isRightCombatAbilities(MOB mob) {
        ...
        for (final Enumeration<Behavior> e = mob.behaviors(); e.hasMoreElements(); ) {
            ...
            if ((B == null) || (B == this))
                return true;
            else if (B instanceof CombatAbilities)
                return false;
        }
        return true;
    }
    ...
    protected Ability useSkill(MOB mob, MOB victim, MOB leader) throws CMException {
        ...
        while ((tryA == null) && ((++tries) < 100) && (mob.numAllAbilities() > 0)) {
            ...
            } else if ((selfQuality == Ability.QUALITY_BENEFICIAL_SELF)
            ...
            if (selfQuality == Ability.QUALITY_BENEFICIAL_SELF)
                target = mob;
            else if (leaderQuality == Ability.QUALITY_BENEFICIAL_OTHERS)
    ...
    public boolean tick(Tickable ticking, int tickID) {
                ...
                if (I instanceof Weapon) {
                    if ((((Weapon) I).minRange() <= rtt) && (((Weapon) I).maxRange() >= rtt)) {
                        if (I.amWearingAt(Wearable.WORN_WIELD))
                            weaponSet.weapon = I;
                        else if (((newWeapon == null) && (weaponSet.weapon == null))
        ...
        if ((!skillUsed)
            ...
            } else if (weaponSet.wand != null) {
                A = ((Wand) weaponSet.wand).getSpell();
                if (A != null) {
                    final MOB target;
                    if (A.castingQuality(mob, mob) == Ability.QUALITY_BENEFICIAL_SELF)
                        target = mob;
                    else if (A.castingQuality(mob, victim) == Ability.QUALITY_MALICIOUS)
                        target = victim;
                    else if (((mob != leader) && (leader != null)) && (A.castingQuality(mob, leader) == Ability.QUALITY_BENEFICIAL_OTHERS))

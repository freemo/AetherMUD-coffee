        ...
        if (this.loyalShipItem != null) {
            switch (msg.sourceMinor()) {
                case CMMsg.TYP_ADVANCE:
                    ...
                    } else if ((msg.target() == loyalShipItem)
                        && (CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS))
                        && (msg.source().riding() != loyalShipItem)) {
                        targetShipItem = (Rideable) msg.target();
                    }
            ...
            if ((ticking instanceof MOB)
                ...
                if ((loyalShipItem == null)
                    ...
                    if ((mob.isMonster())
                        ...
                        else if ((mob.getStartRoom().getArea() != loyalShipArea)
                    ...
                    if ((combatMover)
                        ...
                        while (ourSpeed > 0) {
                            if (ourSpeed == 1) {
                                ...
                                else if ((CMLib.dice().rollPercentage() < 30) && (directionToTarget >= 0))
                                    course.add(CMLib.directions().getDirectionName(directionToTarget));
                                else
                                    course.add(lastDir);
                            } else

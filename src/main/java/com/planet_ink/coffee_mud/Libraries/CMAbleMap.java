        ...
        for (int v = V.size() - 1; v >= 0; v--) {
            ...
            if (V.elementAt(v, 1) instanceof String) {
                ...
                if ((A2 != null) && (A2.proficiency() >= prof.intValue()))
                    V.removeElementAt(v);
                else if (!qualifiesByLevel(studentM, abilityID))
                    V.removeElementAt(v);
            } else {
            ...
            for (int i = 0; i < s.length(); i++) {
                if ((lastC == ' ') && (Character.isLetter(us.charAt(i)))) {
                    ...
                    if (ss.startsWith("MASK=")) {
                        cur = mask;
                        i += 4;
                    } else if (ss.startsWith("PROF=")) {
                        cur = prof;
                        i += 4;
                    } else if (ss.startsWith("REQUIRES=")) {
                        cur = preReqs;
                        i += 8;
                    } else if (ss.startsWith("AUTOGAIN ")) {
                        cur = null;
                        autogain = true;
                        i += 8;
                    } else if (ss.startsWith("AUTOGAIN")

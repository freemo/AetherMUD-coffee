...
public class CMProtocols extends StdLibrary implements ProtocolLibrary {
    ...
    public String getDefaultMXPImage(final Object O) {
        ...
        } else if (O instanceof com.planet_ink.coffee_mud.Items.interfaces.RoomMap)
        ...
        } else if (O instanceof PackagedItems)
            image = getHashedMXPImage(H, "ITEM_PACKAGED");
        else if (O instanceof Perfume)
        ...
        } else if (O instanceof PowerGenerator) {
        ...
        } else if (O instanceof Software)
            image = getHashedMXPImage(H, "ITEM_SOFTWARE");
        else if (O instanceof Technical) {
            String key;
        ...
        } else if (O instanceof Electronics)
            image = getHashedMXPImage(H, "ITEM_ELECTRONICS");
        else if (O instanceof MiscMagic)
    ...
    public byte[] pingMsdp(final Session session, final Map<Object, Object> reportables) {
        try {
            ...
            if (broken == null)
                return null;
            final ByteArrayOutputStream buf = new ByteArrayOutputStream();
            buf.write(Session.TELNET_IAC);
            buf.write(Session.TELNET_SB);
            buf.write(Session.TELNET_MSDP);
            for (final Object var : broken) {
                buf.write(processMsdpSend(session, var.toString()));
            }
            buf.write((char) Session.TELNET_IAC);
            buf.write((char) Session.TELNET_SE);
            return buf.toByteArray();
        } catch (final IOException e) {
            return null;
        }
    ...
    public byte[] processMsdp(final Session session, final char[] data, final int dataSize, final Map<Object, Object> reportables) {
        try {
            ...
            bout.write(Session.TELNET_IAC);
            bout.write(Session.TELNET_SB);
            bout.write(Session.TELNET_MSDP);
            bout.write(buf.toByteArray());
            bout.write(Session.TELNET_IAC);
            bout.write(Session.TELNET_SE);
    ...
    protected String makeGMCPAttribs(final Item I) {
        ...
        if (I.amWearingAt(Item.WORN_WIELD))
            attribs.append("l");
        else if (!I.amWearingAt(Item.IN_INVENTORY))
            attribs.append("w");
        else if ((I.rawProperLocationBitmap() != 0) && (I.rawProperLocationBitmap() != Item.WORN_HELD))
            attribs.append("W");
    ...
    protected String processGmcpStr(final Session session, final String jsonData, final Map<String, Double> supportables) {
        ...
        try {
            ...
            if (pkgSepIndex > 0) {
                ...
                else if (Character.isLetter(jsonDoc.charAt(0)))
                    ...
                    case char_status:
                        if (mob != null) {
                            ...
                            if (session.isAfk())
                                ...
                                state = 9;
                            else if (CMLib.flags().isSitting(mob))
                                state = 11;
                            else if (mob.getVictim() != null)
                                state = 8;
                            else if (mob.isAttributeSet(MOB.Attrib.SYSOPMSGS))
                                state = 6;
                            else if (mob.isAttributeSet(MOB.Attrib.AUTORUN))
                                state = 12;

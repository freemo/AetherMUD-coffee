...
import com.planet_ink.coffee_mud.core.collections.QuadVector;
...
public class CMAbleComps extends StdLibrary implements AbilityComponents {
    ...
    public boolean IsItemComponent(MOB mob, AbilityComponent comp, int[] amt, Item I, List<Object> thisSet, final boolean mithrilOK) {
        ...
        if ((comp.getType() == AbilityComponent.CompType.STRING)
            ...
            return false;
        else if ((comp.getType() == AbilityComponent.CompType.RESOURCE)
            && ((!(I instanceof RawMaterial)) || (I.material() != comp.getLongType())))
            return false;
        else if ((comp.getType() == AbilityComponent.CompType.MATERIAL)
        ...
        } else if (I.phyStats().weight() > amt[0]) {
            I = (Item) CMLib.materials().unbundle(I, amt[0], null);
            if (I == null)
                return false;
    ...
    public PairList<String, String> getAbilityComponentCoded(AbilityComponent comp) {
        ...
        else if (comp.getLocation() == AbilityComponent.CompLocation.WORN)
            curr.add("DISPOSITION", "worn");
        else if (comp.getLocation() == AbilityComponent.CompLocation.NEARBY)
            curr.add("DISPOSITION", "nearby");
        else if (comp.getLocation() == AbilityComponent.CompLocation.ONGROUND)
            curr.add("DISPOSITION", "onground");
        else if (comp.getLocation() == AbilityComponent.CompLocation.INVENTORY)
    ...
    public void setAbilityComponentCodedFromCodedPairs(PairList<String, String> decodedDV, AbilityComponent comp) {
        ...
        else if (s[1].equalsIgnoreCase("worn"))
            comp.setLocation(AbilityComponent.CompLocation.WORN);
        else if (s[1].equalsIgnoreCase("nearby"))
            comp.setLocation(AbilityComponent.CompLocation.NEARBY);
        else if (s[1].equalsIgnoreCase("onground"))
    ...
    public String getAbilityComponentDesc(MOB mob, AbilityComponent comp, boolean useConnector) {
        ...
        if (comp.getLocation() == AbilityComponent.CompLocation.INVENTORY)
            buf.append(itemDesc);
        else if (comp.getLocation() == AbilityComponent.CompLocation.HELD)
    ...
    public String addAbilityComponent(String s, Map<String, List<AbilityComponent>> H) {
        ...
        while (parms.length() > 0) {
            ...
            else if (parmS.substring(0, x).equalsIgnoreCase("worn"))
                build.setLocation(AbilityComponent.CompLocation.WORN);
            else if (parmS.substring(0, x).equalsIgnoreCase("nearby"))
                build.setLocation(AbilityComponent.CompLocation.NEARBY);
            else if (parmS.substring(0, x).equalsIgnoreCase("onground"))
                build.setLocation(AbilityComponent.CompLocation.ONGROUND);
            else if ((x > 0) && (!parmS.substring(0, x).equalsIgnoreCase("inventory"))) {
            ...
            else if ((x > 0) && (!parmS.substring(0, x).equalsIgnoreCase("consumed"))) {
    ...
    public void alterAbilityComponentFile(String compID, boolean delete) {
        ...
        for (int t = 0; t < text.length(); t++) {
            if (text.charAt(t) == '\n')
                lastWasCR = true;
            else if (text.charAt(t) == '\r')
                lastWasCR = true;
            else if (Character.isWhitespace(text.charAt(t)))
                continue;
            else if ((lastWasCR) && (delFromHere >= 0)) {
                text.delete(delFromHere, t);
                text.insert(delFromHere, parms + '\n');
                delFromHere = -1;
                addIt = false;
                break;
            } else if ((lastWasCR) && (Character.toUpperCase(text.charAt(t)) == upID.charAt(0))) {
    ...
    public AbilityLimits getSpecialSkillRemainders(MOB student) {
        ...
        for (int a = 0; a < student.numAbilities(); a++) {
            ...
            } else if (((A2.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_LANGUAGE)
                && (!A2.ID().equals("Common"))) {
                if (culturalAbilities.contains(A2.ID().toLowerCase()))
                    continue;
                boolean foundInAClass = false;

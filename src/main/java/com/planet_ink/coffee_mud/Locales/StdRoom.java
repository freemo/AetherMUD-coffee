...
public class StdRoom implements Room {
    ...
    public int getAtmosphere() {
        ...
        else if (getAtmosphereCode() != ATMOSPHERE_INHERIT)
    ...
    public int getClimateType() {
        ...
        else if (getClimateTypeCode() != CLIMASK_INHERIT)
    ...
    public void setDescription(String newDescription) {
        if (newDescription.length() == 0)
            description = null;
        else if (CMProps.getBoolVar(CMProps.Bool.ROOMDCOMPRESS))
    ...
    public void executeMsg(final Environmental myHost, final CMMsg msg) {
        ...
        if (msg.sourceMinor() == CMMsg.TYP_SHUTDOWN) {
            try {
                ...
                } else if (CMSecurity.isSaveFlag(CMSecurity.SaveFlag.ROOMSHOPS)) {
                    eachInhabitant(new EachApplicable<MOB>() {
                        @Override
                        public final void apply(final MOB M) {

...
public class Destroy extends StdCommand {
    ...
    public boolean execute(final MOB mob, List<String> commands, final int metaFlags)
        ...
        if ((!CMSecurity.isAllowedAnywhereContainsAny(mob, CMSecurity.SECURITY_CMD_GROUP))
                ...
                if ((dropThis == null)
                    ...
                    if (dropThis != null) {
                        ...
                        if ((matType != RawMaterial.MATERIAL_GLASS)
                            ...
                            return false;
                        } else if ((!dropThis.amWearingAt(Wearable.WORN_HELD)) && (!dropThis.amWearingAt(Wearable.WORN_WIELD))) {
            ...
            for (int i = 0; i < V.size(); i++) {
                if (destroyItem(mob, V.get(i), false, true))
                    didAnything = true;
                else if (V.get(i) instanceof Coins)
                    ((Coins) V.get(i)).putCoinsBack();
                else if (V.get(i) instanceof RawMaterial)
        ...
        if (commandType.equals("EXIT")) {
        ...
        } else if (commandType.equals("ABILITY") || commandType.equals("LANGUAGE") || commandType.equals("CRAFTSKILL")) {
        ...
        } else if (commandType.equals("DISABLEFLAG")) {
            ...
            return false;
        } else if (commandType.equals("ENABLEFLAG")) {
            if (!CMSecurity.isAllowed(mob, mob.location(), CMSecurity.SecFlag.LISTADMIN))
                return errorOut(mob);
            final String named = CMParms.combine(commands, 2);
            ...
            return false;
        } else if (commandType.equals("DEBUGFLAG")) {
            if (!CMSecurity.isAllowed(mob, mob.location(), CMSecurity.SecFlag.LISTADMIN))
                return errorOut(mob);
            final String named = CMParms.combine(commands, 2);
            ...
            return false;
        } else if (commandType.equals("NOPURGE")) {
        ...
        } else if (commandType.equals("TICKS")) {
        ...
        } else if (commandType.equals("JOURNAL")) {
            ...
            else if (mob.session().confirm(L("This will destroy all @x1 messages.  Are you SURE (y/N)? ", "" + CMLib.database().DBCountJournal(name, null, null)), "N")) {
            ...
            if (commands.size() < 3)
                ...
                else if ((!mob.isMonster()) && (mob.session().confirm(L("Destroy faction '@x1' -- this could have unexpected consequences in the future -- (N/y)? ", F.factionID()), "N"))) {
                    try {
        ...
        } else if (commandType.equals("WEBSERVER")) {
            if (!CMSecurity.isASysOp(mob))
                return errorOut(mob);
            if (commands.size() < 3) {
            ...
            if (commands.size() < 3)
                ...
                else if (CMLib.clans().removeGovernment(G)) {

...
public class CommonSkill extends StdAbility {
        ...
        if (resources != null)
            for (int i = 0; i < resources.size(); i++) {
                if (isMaterial && (resource == (resources.get(i).intValue() & RawMaterial.MATERIAL_MASK)))
                    return true;
                else if (resources.get(i).equals(I))
                    return true;
            }
        return false;
    }
    ...
    public void teach(MOB teacher, MOB student) {
        ...
        if ((student != null) && (student.fetchAbility(ID()) != null)) {
            ...
            else if (remainders.commonSkills() <= Integer.MAX_VALUE / 2)
            ...
            else if (remainders.specificSkillLimit() <= Integer.MAX_VALUE / 2)

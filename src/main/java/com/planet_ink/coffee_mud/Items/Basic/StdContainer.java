...
import com.planet_ink.coffee_mud.core.collections.ReadOnlyList;
                ...
                case CMMsg.TYP_PUT:
                    if (msg.tool() instanceof Item) {
                        ...
                        if (hasADoor() && (!isOpen())) {
                            ...
                            return false;
                        } else if (newitem.amWearingAt(Wearable.WORN_WIELD)) {
                            mob.tell(L("You are already wielding that!"));
                            return false;
                        } else if (newitem.amWearingAt(Wearable.WORN_HELD)) {
                            mob.tell(L("You are holding that!"));
                            return false;
                        } else if (!newitem.amWearingAt(Wearable.IN_INVENTORY)) {
                            ...
                            if (!canContain(newitem)) {
                                ...
                                return false;
                            } else if (newitem.phyStats().weight() > capacity) {
                                mob.tell(L("@x1 won't fit in @x2.", newitem.name(), name()));
                                return false;
                            } else if ((recursiveWeight() + newitem.phyStats().weight()) > capacity) {
                                if (!hasContent())
                                    mob.tell(L("@x1 is too small.", name()));
                                else if ((newitem instanceof Software) && (this instanceof Computer))
                    ...
                    if (msg.tool() instanceof Item) {
                        ...
                        if (newitem.container() == this) {
                            if ((!(CMLib.flags().canBeSeenBy(newitem, mob) || (newitem instanceof Light)))
                                ...
                                return false;
                            } else if (hasADoor() && (!isOpen())) {
                                mob.tell(L("@x1 is closed.", name()));
                                return false;
                            } else if ((mob.phyStats().level() < newitem.phyStats().level() - (10 + (mob.phyStats().level() / 5)))
                                && (!(mob instanceof ShopKeeper))
                                && (!mob.charStats().getMyRace().leveless())
                                && (!mob.charStats().getCurrentClass().leveless())) {
                                mob.tell(L("@x1 is too powerful to endure possessing it.", newitem.name()));
                                return false;
                            } else if ((newitem.recursiveWeight() > (mob.maxCarry() - mob.phyStats().weight())) && (!mob.isMine(this))) {
                                mob.tell(L("@x1 is too heavy.", newitem.name()));
                                return false;
                            } else if ((newitem.numberOfItems() > (mob.maxItems() - mob.numItems())) && (!mob.isMine(this))) {
                                mob.tell(L("You can't carry that many items."));
                                return false;
                            } else if (!CMLib.flags().isGettable(newitem)) {
                        ...
                        mob.tell(mob, newitem, this, L("You can't see <T-NAME> in <O-NAME>."));
                        return false;
                    } else if ((recursiveWeight() > (mob.maxCarry() - mob.phyStats().weight())) && (!mob.isMine(this))) {
                        mob.tell(L("@x1 is too heavy.", name()));
                        return false;
                    } else if (((numberOfItems() > mob.maxItems() - mob.numItems())) && (!mob.isMine(this))) {
                        mob.tell(L("You can't carry that many items."));
                        return false;
                    }
                    break;
                ...
                case CMMsg.TYP_UNLOCK:
                    ...
                    if (isOpen) {
                        ...
                        if ((!isLocked) && (msg.targetMinor() == CMMsg.TYP_UNLOCK)) {
                            ...
                            return false;
                        } else if ((isLocked) && (msg.targetMinor() == CMMsg.TYP_LOCK)) {

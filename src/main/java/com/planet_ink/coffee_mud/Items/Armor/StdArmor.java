...
public class StdArmor extends StdContainer implements Armor {
    ...
    public SizeDeviation getSizingDeviation(MOB mob) {
        ...
        if ((phyStats().height() > 0)
            ...
            if ((rawProperLocationBitmap() & SMALL_DEVIATION_WEAR_LOCS) > 0)
                devianceAllowed = 20;
            else if ((rawProperLocationBitmap() & STRANGE_DEVIATION_WEAR_LOCS) > 0) {
                ...
                if (wcode == Wearable.WORN_FLOATING_NEARBY)
                    devianceAllowed = -1;
                else if (wcode == Wearable.WORN_MOUTH)
                    devianceAllowed = -1;
                else if (wcode == Wearable.WORN_EYES)
                    devianceAllowed = 1000;
                else if (wcode == Wearable.WORN_EARS)
                    devianceAllowed = 1000;
                else if (wcode == Wearable.WORN_NECK)
        ...
        if ((msg.amITarget(this))
            ...
            if (deviation != Armor.SizeDeviation.FITS) {
                if (deviation == Armor.SizeDeviation.TOO_LARGE) {
                    ...
                    return false;
                } else if (deviation == Armor.SizeDeviation.TOO_SMALL) {
    ...
    public void executeMsg(final Environmental myHost, final CMMsg msg) {
        ...
        else if ((!amWearingAt(Wearable.IN_INVENTORY))
            && (owner() != null)
    ...
    public void affectPhyStats(Physical affected, PhyStats affectableStats) {
        ...
        if ((!amWearingAt(Wearable.IN_INVENTORY))
            ...
            if (phyStats().armor() != 0) {
                ...
                else if ((amWearingAt(Wearable.WORN_HEAD)) || (this.amWearingAt(Wearable.WORN_HELD)))
                    affectableStats.setArmor(affectableStats.armor() - (ability * 2));
                else if (!amWearingAt(Wearable.WORN_FLOATING_NEARBY))

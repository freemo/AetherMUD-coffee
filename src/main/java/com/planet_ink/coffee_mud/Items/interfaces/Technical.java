    ...
    public static enum TechCommand {
        ...
        public String makeCommand(Object... parts) {
            ...
            for (int i = 0; i < parms.length; i++) {
                if (parts[i] == null)
                    return "";
                else if (parms[i] == String[].class) {
                    for (; i < parms.length; i++)
                        str.append(" ").append(parts[i].toString());
                    break;
                } else if (!parms[i].isAssignableFrom(parts[i].getClass()))
                    return "";
                else
                    str.append(" ").append(parts[i].toString());
            }
        ...
        public Object[] confirmAndTranslate(String[] parts) {
            ...
            for (int i = 0; i < parms.length; i++) {
                if (parms[i].isEnum()) {
                    ...
                    if (resp[i] == null)
                        return null;
                } else if (Integer.class.isAssignableFrom(parms[i]) || Long.class.isAssignableFrom(parms[i])) {
                ...
                } else if (Double.class.isAssignableFrom(parms[i]) || Float.class.isAssignableFrom(parms[i])) {
                ...
                } else if (Boolean.class.isAssignableFrom(parms[i])) {
                    if (!CMath.isBool(parts[i + 1]))
                        return null;
                    resp[i] = Boolean.valueOf(parts[i + 1]);
                } else if (String.class.isAssignableFrom(parms[i])) {
                    resp[i] = parts[i + 1];
                } else if (String[].class.isAssignableFrom(parms[i])) {

...
public class MXP {
    ...
    public int processTag(StringBuffer buf, int i) {
        ...
        while ((bit != null) && ((++i) < buf.length())) {
            switch (buf.charAt(i)) {
                ...
                case '\'':
                    ...
                    } else if ((lastC == '=') || (quotes != '\0') || ((quotes == '\0') && ((lastC == ' ') || (lastC == '\t')))) {
        ...
        if (endTag) {
            ...
            if (E.needsText()) {
                ...
                return close.length();
            } else if (close.equals("</" + E.name())) {
                if (tagDebug) {
                    System.out.println("/TAG*/END/4/adji=!2.5!/" + (close.length() - 1));
                    System.out.flush();
                }
                return close.length();
            } else if (E.getBufInsert() < oldI) {
                if (tagDebug) {
    ...
    private void specialProcessorElements(MXPElement E, boolean endTag) {
        if (E.name().equals("FONT")) {
            ...
            if ((style != null) && (color == null) && (back == null) && (face == null) && (size == null)) {
                ...
                while (style.length() > 0) {
                    ...
                    if (y >= 0) {
                        ...
                        else if (s.equalsIgnoreCase("font-size"))
                            E.setAttributeValue("SIZE", v);
                        else if (s.equalsIgnoreCase("font-family"))
        ...
        } else if (E.name().equals("ELEMENT") || E.name().equals("EL")) {
            ...
            return;
        } else if (E.name().equals("ENTITY") || E.name().equals("EN")) {
            final String name = E.getAttributeValue("NAME");
            ...
            return;
        } else if ((E.name().equals("VAR") || E.name().equals("V")) && (endTag)) {
            final String name = E.getAttributeValue("NAME");
            // String PRIVATE=E.getAttributeValue("PRIVATE");
            // String PUBLISH=E.getAttributeValue("PUBLISH");
            final String DELETE = E.getAttributeValue("DELETE");
        ...
        } else if (E.name().equalsIgnoreCase("ATTLIST") || E.name().equalsIgnoreCase("ATT")) {
            final String name = E.getAttributeValue("NAME");
        ...
        if ((i < buf.length() - 3) && (buf.charAt(i + 1) == '#') && (Character.isDigit(buf.charAt(i + 2)))) {
            ...
            while ((++i) < buf.length()) {
                if (buf.charAt(i) == ';') {
                    convertIt = false;
                    break;
                } else if (!Character.isLetterOrDigit(buf.charAt(i))) {
                    convertIt = true;
                    break;
                } else if ((!Character.isLetter(buf.charAt(i))) && (content.length() == 0)) {
                    convertIt = true;
                    break;
                }

...
public class TelnetFilter {
                    ...
                    switch (buf.charAt(++i)) {
                        case IAC_SB: {
                            ...
                            if (subOptionCode == TELOPT_TTYPE) {
                                ...
                                response.flush();
                            } else if (subOptionCode == TELOPT_NAWS) {
                                if (debugTelnetCodes)
                                ...
                                response.writeBytes("" + IAC_ + IAC_SE);
                                response.flush();
                            } else if (subOptionCode == MCCP_COMPRESS2) {
                                // probably need to handle this earlier
                            } else if (subOptionCode == IAC_MSDP) {
                            ...
                            } else if (subOptionCode == IAC_GMCP) {
                        ...
                        case IAC_WILL:
                            ...
                            } else if (buf.charAt(i) == MCCP_COMPRESS2) {
                                if (debugTelnetCodes)
                            ...
                            } else if (buf.charAt(i) == TELOPT_LOGOUT) {
                                // goot for you serverdude
                            } else if (buf.charAt(i) != TELOPT_BINARY) {
                                if (debugTelnetCodes)
    ...
    public int HTMLFilter(StringBuffer buf) {
        ...
        while (i < buf.length()) {
            ...
            } else if ((MXPsupport() && (mxpModule.eatTextUntilNextEOLN()) && (buf.charAt(i) != '\n') && (buf.charAt(i) != '\r'))) {
                buf.deleteCharAt(i);
                i--;
            } else
                switch (buf.charAt(i)) {
                    ...
                    case '<':
                        if (!MXPsupport()) {
                            ...
                            buf.insert(i + 1, "lt;");
                            i += 3;
                        } else if (((i + 4) < buf.length()) && (buf.substring(i + 1, i + 4).equals("!--")))
                            comment = true;
                        else {
                        ...
                        if (buf.charAt(++i) != '[')
                            ...
                            if (translate.equals(oldStr)) {
                                ...
                                if (x == Integer.MAX_VALUE)
                                    return i;
                                i = savedI + x;
                            } else if (!translate.equals(oldStr)) {
        ...
        try {
            if (data.startsWith("\\")) {
                ...
                if (cmd.equalsIgnoreCase("MSDP")) {
                    ...
                    return null;
                } else if (cmd.equalsIgnoreCase("GMCP")) {
                    try {
